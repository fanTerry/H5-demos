...
<!-- 单关 -->
<input type="hidden" id="jq_play_type" value="dg">
			...
			<c:forEach items="${groupVo.matchVoList}" var="matchVo">
				...
				<tr data-leaguename="${matchVo.leagueName}" data-fxid="${matchVo.fxId}" data-matchid="${matchVo.matchId}" data-matchno="${matchVo.uniqueMatchNo}" class="Jq_line_match">
					...
					<td class="game_sheng gm_bgp Jq_sp">
						<p data-index="0" data-sp="${empty spf_3 ? null : '3-'.concat(spf_3)}">
							${empty spf_3 ? '-' : spf_3}
						</p>
                		<p data-index="0" data-sp="${empty rqspf_3 ? null : '3-'.concat(rqspf_3)}">
							${empty rqspf_3 ? '-' : rqspf_3}
						</p>
					</td>
					<td class="game_ping gm_bgp Jq_sp">
						<p data-index="1" data-sp="${empty spf_1 ? null : '1-'.concat(spf_1)}">
							${empty spf_1 ? '-' : spf_1}
						</p>
                		<p data-index="1" data-sp="${empty rqspf_1 ? null : '1-'.concat(rqspf_1)}">
							${empty rqspf_1 ? '-' : rqspf_1}
						</p>
					</td>
					<td class="game_fu gm_bgp Jq_sp">
						<p data-index="2" data-sp="${empty spf_0 ? null : '0-'.concat(spf_0)}">
							${empty spf_0 ? '-' : spf_0}
						</p>
                		<p data-index="2" data-sp="${empty rqspf_0 ? null : '0-'.concat(rqspf_0)}">
