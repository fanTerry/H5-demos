    ...
    <div class="content clearfix">
        ...
        <div class="aside">
            <div class="wrap1080 ppd70 fz14">
                <p class="pu_title bor_bot0">专栏文章</p>
                <!-- 查询 -->
                <div class="clearfix fb_title ml30">
                    <div class="fl pr pdr100">
                        ...
                        <span class="f_span2">文章标题</span>
                    	<input type="text" id="title" class="input01">
	...
	<tbody>
		...
		{{~it.dataList :item:index}}
			<tr>
				...
				<td>
					<a href="/column/showArticle/{{=item.id}}" style="color: #0082ff;">详情</a> 
					| <a href="/column/editArticle/{{=item.id}}" style="color: #0082ff;">编辑</a> 
					{{? item.isFree ==0}}| <a href="javascript:void(0)" id="btnSetFree" data-id="{{=item.id}}" data-free="1" style="color: #0082ff;">设为试读</a> {{?}}
					{{? item.isFree ==1}}| <a href="javascript:void(0)" id="btnSetFree" data-id="{{=item.id}}" data-free="0" style="color: #0082ff;">取消试读</a> {{?}}
				</td>
			</tr>
		{{~}}
	</tbody>
...
$(document).ready(function(){
	...
	//设置试读
	$(document).on("click", "#listContainer #btnSetFree", function(){
		var _this = this;
		var free = $(_this).data("free");
		var confirmMsg = '确定要取消试读吗？';
		if(free==1)
		{
			confirmMsg = '确定要设为试读吗？';
		}
		...
		var freeVar = free==1?'true':'false';
		$.getJSON("/column/setArticleFree/" + id+"/"+freeVar, function(json){
			$(_this).data('isLock', 0);		
	...
	$(document).on("click", "#listContainer #btnDelete", function(){
		...
		$.getJSON("/column/deleteArticle/" + id, function(json){
			$(_this).data('isLock', 0);		
