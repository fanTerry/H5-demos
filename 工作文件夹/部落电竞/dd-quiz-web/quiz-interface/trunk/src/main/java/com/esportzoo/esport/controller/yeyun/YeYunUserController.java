...
public class YeYunUserController extends BaseController {
	...
	public CommonResponse<List<String>> getYeYunUrlList(HttpServletRequest request){
		...
		try {
			...
			//未绑定
			if ("user.null".equals(modelResult.getErrorCode())){
				logger.info("{}获取椰云登录token，{}，{}前往绑定",logPrefix,userConsumer.getNickName(),userId);
				ModelResult<YeYunUserInfo> yeYunUserInfoModelResult = yeYunUserServiceClient.bindUser(userId);
				if (!yeYunUserInfoModelResult.isSuccess() || yeYunUserInfoModelResult.getModel() == null){
					logger.info("{}绑定椰云用户错误，参数：{}，errorMsg：{}", logPrefix, userId, modelResult.getErrorMsg());
					return CommonResponse.withErrorResp("绑定椰云用户错误");
				}
				//再次获取token
				modelResult = yeYunUserServiceClient.getTokenAndIndexUrl(userId);
			}
	...
	public CommonResponse<YeYunUserInfoResponse> yeYunUserInfo(HttpServletRequest request){
		...
		try {
			...
			if (CollectionUtil.isEmpty(userThirds)){
				//用户未绑定，前往绑定
				logger.info("{}获取用户信息，{}，{}前往绑定",logPrefix,userConsumer.getNickName(),userId);
				...
				if (yeYunUserInfoResult.getModel() == null || StringUtils.isEmpty(yeYunUserInfoResult.getModel().getUserId())){
					logger.info("{}获取用户信息，用户未绑定时去绑定发生异常",logPrefix,yeYunUserInfoResult.getErrorMsg());
				...
				logger.info("{}获取用户信息，用户状态异常userThirds：{}",logPrefix, JSON.toJSONString(userThirds));
