...
if (window.localStorage.token) {
    if (!getCookie("ddquiz_h5_login_cookie_sid")) {
        console.log("进来获取本地缓存的token");
        var token = window.localStorage.token
        var day = 1;
        var exp = new Date();
        var name = "ddquiz_h5_login_cookie_sid";
...
router.beforeEach((to, from, next) => {
    ...
    //     console.log("分享码shareCode", shareCode);
    // }
    var urlParam = convertObjToUrlParam(to.query);
    console.log("拼接URL携带的参数：", urlParam);
    ...
    //     for (var i = 0; i < aCookie.length; i++) {
    //         var aCrumb = aCookie[i].split("=");
    //         if ("ddquiz_ubox_login_cookie_sid" == aCrumb[0]) {
    ...
    //         window.location.href = "/api/third/login?" + urlParam + "&redirect=" + encodeURIComponent(window.location.href);
    ...
    if (clientType == 7) { //微信公众号进入
        ...
        for (var i = 0; i < aCookie.length; i++) {
            var aCrumb = aCookie[i].split("=");
            if ("ddquiz_wx_account_login_cookie_sid" == aCrumb[0]) {
                cookieSid = unescape(aCrumb[1]);
            }
        }
