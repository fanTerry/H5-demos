  ...
  <div class="ui_pop">
    <div class="pop_pay">
      ...
      <div class="att_tips" v-for="item in chargeWayList">
        <template v-if="item.payIndex==2"> 温馨提示:1元友宝余额价值与1000星星价值一致</template>
      </div>
    </div>
  </div>
</template>
...
export default {
    ...
    if (_self.chargeWayList) {
      _self.chargeWayList.forEach(function(item) {
        ...
        if (parseFloat(item.showBalance) >= parseFloat(_self.price)) {
          ...
          // if (_self.noBalanceList.indexOf(_self.choosedPayWay.index) > -1) {
          // } else {
          //   if (parseFloat(item.balance) < parseFloat(_self.price)) {
          //     _self.buttonText = "余额不足,去充值";
          //     _self.notEnough = true;
          //   }
          // }
        }
      });
  ...
  methods: {
    ...
    choose(item) {
      ...
      // if (this.noBalanceList.indexOf(this.choosedPayWay.index) > -1) {
      //   this.buttonText = "确认";
      //   this.notEnough = false;
      //   return;
      // }
      // if (parseFloat(item.balance) > parseFloat(this.price)) {
      //   this.buttonText = "余额不足,去充值";
      //   this.notEnough = true;
      // }
    },
...
.pop_pay {
  ...
  .pay_method {
    ...
    .selected {
      @include getBgImg("../../../assets/images/common/select.png");
      border: none;
    }
  }
  .yb_icon {
