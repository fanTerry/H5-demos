  ...
  <div class="Page guessPage" v-if='readyFlag'>
      ...
      <div class="navBar">
        <div class="users">
          ...
          <div class="coco_points" @click="showExchangeType = true">
            <p>椰子分 / 嗨乐分</p>
            <a>{{userInfo.yeYunPoints?userInfo.yeYunPoints:'0'}} / {{userInfo.haiLePoints?userInfo.haiLePoints:'0'}}
            </a>
          </div>
        </div>
    ...
    <div class="main" id="mainId">
      ...
      <mescroll ref="mescroll" :class="{'ban_slide':showExchangeType||showSignType||getBetData.toCurBet}"
        @downCallback="downCallback" @upCallback="upCallback" @mescrollInit="mescrollInit" :isShowEmpty="false">
        <!-- 运营位 -->
        ...
        <!-- 没有数据时展示 -->
        <no-data v-if="noData" :text="'暂时没有新的比赛'"></no-data>
  ...
  methods: {
    ...
    toPop(location) {
      ...
      if (location == 1) {
        ...
        // this.showSignType = true;  //注释掉banner图入口
      } else if (location == 3) {
        this.openWinTop();
      }
    }
