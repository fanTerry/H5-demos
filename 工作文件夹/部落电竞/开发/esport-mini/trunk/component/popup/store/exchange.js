      ...
      api._postAuth("/shopOrder/submit/" + this.data.goodsDetail.id, param)
        .then(res => {
          ...
          if ((res.code = "200" && res.data != null)) {
            ...
            setTimeout(() => {
              wx.redirectTo({
                url: '/pages/store/order/orderDetails?orderId=' + res.data.outOrderId
              })
            }, 500);
      ...
      wx.requestPayment({
        ...
        'success': function (res) {
          ...
          if (res.errMsg == "requestPayment:ok") { // 调用支付成功
            ...
            setTimeout(() => {
              wx.redirectTo({
                url: '/pages/store/order/orderDetails?orderId=' + result.outOrderId
              })
            }, 2000);
