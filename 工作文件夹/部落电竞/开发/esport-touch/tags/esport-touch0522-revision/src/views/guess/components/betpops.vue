<template>
  <div class="ui_pop" v-if="showBetOuterFlag&&betNumFlag">
    ...
    <div class="bet">
      ...
      <p class="tips">
        下单消耗 {{userBetNum ||0}} 星星，预测成功可得</p>
      <p class='awards'>{{parseFloat((userBetNum * (selectItem&&selectItem.odds?selectItem.odds:0)).toFixed(2))}}</p>
      <div class="bet_num">
        <ul>
          <li :class="{select:userBetNum == num&&!focusFlag}" v-for="(num,index) in betNumList" :key="index"
            @click="chooseBetNum(num,$event)">
            <template v-if='index==2&&maxPreBetNum>0&&userInfo.ableRecScore >= maxPreBetNum'><span
                style='font-size: 2.8vw;'>最高</span></template>
...
export default {
  ...
  methods: {
    ...
    getPreBetNum() {
      this.$post("/api/quiz/bet/getPreBetting", {
        ...
        .then(res => {
          ...
          if (res.code == 200 && res.data && res.data[this.selectItem.index]) {
            this.betNumFlag = true;
            this.maxPreBetNum = res.data[this.selectItem.index];
    ...
    checkBalance() {
      ...
      if (this.maxPreBetNum > 0 && this.userBetNum > this.maxPreBetNum * 0.9) {
        //此选项上用户最多能投多少钱
        this.$toast("该选项最多可投" + this.maxPreBetNum * 0.9 + "星星", 4);
        return false;
      }
      return true;
    },
