<template>
  <div class="Page gameTopicPage">
    ...
    <div class="main" id="mainId" :class="{'flex_v_h':noData,'isIosApp':isIos}">
      ...
      <mescroll id="mescroll" ref="mescroll" :downLoadAuto="false" @downCallback="downCallback" @upCallback="upCallback" @mescrollInit="mescrollInit" @handleScroll='handleMescrollY'
        :isShowEmpty="false">
        <div class="topic_header" :style="topicHeaderInfo.imageUrl!=null?'background-image:url('+topicHeaderInfo.imageUrl+')':'background-image:url('+globalConst.topicDefaultImgUrl+')'">
          ...
          <div class="topic_title">
            ...
            <!-- followed 已关注 -->
            <span @click="followTopic(topicHeaderInfo.id)" :class="{'follow':!topicHeaderInfo.attention,'followed':topicHeaderInfo.attention}">{{topicHeaderInfo.attention?'- 已关注':'+ 关注'}}</span>
          </div>
        ...
        <article-tab v-if="!noData" :showType='showType' :cmsContentList='cmsContentList'></article-tab>
      </mescroll>
      <!-- </scroll> -->
  ...
  methods: {
    weiXinSahare() {
      this.$wxApi.wxRegister({
        title: '赢加竞技-' + this.topicHeaderInfo.name,
        ...
        .then(rsp => {
          ...
          if (dataResponse.code == '200') {
            ...
            if (dataResponse.data.length > 0) {
              this.cmsContentList = this.cmsContentList.concat(dataResponse.data);
            }
    ...
    handleMescrollY() {
      ...
      let topicHeaderHeight = document.querySelector('.topic_header').clientHeight;
...
.topic_title {
  ...
  .followed {
    ...
    right: 5px;
    bottom: 18px;
    @include getBtn(auto, 23px, 12px, #fff, $color_btn, 23px);
    padding: 0 10px;
  }
  .followed {
    background: #fff;
