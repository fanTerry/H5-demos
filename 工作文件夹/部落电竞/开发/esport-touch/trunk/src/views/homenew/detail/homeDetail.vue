  ...
  <div ref="homeDetailPage" class="Page homeDetail_Page" v-if="cmsDetail != null">
    <header class="mod_header">
      ...
      <figure class="figure_film" v-if="type==2">
        ...
        <xi-gua-video :videoUrl="cmsDetail.content" :poster='require("../../../assets/images/common/video_default_image.png")' :videoId="cmsDetail.id" type='cmsdetail' scene='detail'></xi-gua-video>
      ...
      <mescroll ref="mescroll" :downLoadAuto="false" :isUseDown="false" @upCallback="onPullingUp" @mescrollInit="mescrollInit">
        <section class="mod_userInfo">
          <div class="user_info">
            <div>
              <!--不是短文默认头像用赢加竞技-->
              <img preview='0' v-if='!judeIsJuzi()&&type!=3' :src="cmsDetail.authorIcon|getDefaultImg('https://rs.esportzoo.com/svn/esport-res/mini/images/default/winplus.png')" alt />
              <!--是短文默认头像用用户默认头像-->
              <img preview='0' v-if='!judeIsJuzi()&&type==3' :src="cmsDetail.authorIcon|getDefaultImg(globalConst.userDefaultIcon)" alt />
              <img preview='0' v-if='judeIsJuzi()' src="https://rs.esportzoo.com/svn/esport-res/mini/images/default/juzi.png" alt />
              <div>
                <div class="name" v-if='!judeIsJuzi()'>{{cmsDetail.authorName==null?'赢加竞技':cmsDetail.authorName}}</div>
                <div class="name" v-else>橘子电竞</div>
                <!-- <div class="name" else>赢加竞技</div> -->
                <p class="tips">
        ...
        <section class="parag_comments">
          ...
          <template v-if="upTopCommentList.length>0">
            <div class="item" v-for="(dataItem,index) in upTopCommentList" :key="index+'cms1'">
              <article-discuss-item :ref="'myChildHot'+dataItem.commentId" :commentItem="dataItem" :hotFlag="1" @toReplyFocus="replyFocus" @toDetailedComment="toDetailedComment"
                :replyType="replyType=1" :floorType="true" @toDelComment="toDelComment">
          ...
          <div class="item" v-for="(dataItem,index) in commentList" :key="index+'cms2'">
            <article-discuss-item :ref="'myChild'+dataItem.commentId" :commentItem="dataItem" :hotFlag="0" @toReplyFocus="replyFocus" @toDetailedComment="toDetailedComment" :replyType="replyType=1"
    ...
    <!-- 二级回复评论 -->
    <comment-pop :cmsDetail="cmsDetail" :class="{'fadeToTop':showComment}" v-if="showComment" ref="commentPop" :contentId="contentId" :contentTypeId="contentTypeId" @toDelComment="toDelComment">
    </comment-pop>
      ...
      return this.$post('/api/cmsComment/delUserComment', {
        ...
        .then(res => {
          ...
          if (dataResponse.code == 200) {
            ...
            //当在详细页删除评论时,刷新数据
            if (this.parentCommentId != undefined && this.parentCommentId != this.delCommentId) {
        ...
        .then(dataResponse => {
          ...
          if (dataResponse.code == '200') {
            ...
            this.$wxApi.wxRegister({
              title: titleShare,
              desc: '赢加竞技，全球最新游戏资讯，精彩赛事直播，电竞玩家聚集地。',
