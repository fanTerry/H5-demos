<template>
  <div class="Page allGuessPage">
    ...
    <!-- 直播 -->
    <div class="live" v-if="matchInfo.liveVisible==1 &&activatedFlag && matchInfo.liveVideoUrlList && matchInfo.matchStatus==1" >
      <!-- <video-live :videoLiveInfo="videoLiveInfo" :videoUrl="videoLiveInfo.videoUrl" :videoId="videoLiveInfo.videoId"></video-live> -->
      <videoAliPlayer :videoUrl="videoLiveInfo.videoUrl" ></videoAliPlayer>
    </div>

    <div class="tab">
      <!-- <span>数据</span> -->
      <span :class="{active:tabType==1}" @click="changeTabType(1)">预测</span>
      <span :class="{active:tabType==2}" @click="changeTabType(2)">跟单</span>
      <!-- <span>方案</span> -->
    </div>
    <div class="main" :class="{'ban_slide':getBetData.toCurBet}">
      <!-- 预测板块 -->
      <section class="mod_guess" v-if="tabType==1">
        <ul class="guess_list">
        ...
        <no-data v-if="moreNoData && endNoData" :text="'暂无盘口，敬请期待'"> </no-data>
      </section>
      <!-- 方案与跟单板块 -->
      <plan-and-project ref="followPlan" :type="2" :matchId='matchId' v-if="tabType==2"></plan-and-project>
...
import videoLive from '../../components/common/videoLive.vue';
import videoAliPlayer from '../../components/common/videoAliPlayer';

import planAndProject from './components/planAndProject.vue';
...
export default {
  ...
  activated() {
    ...
    if (this.$route.meta.isBack) {
      ...
      this.tabType = 1;
      setTimeout(() => {
        console.log('当前视频对象', this.videoLiveInfo);
      }, 300);
    }
  ...
  deactivated() {
    ...
    console.log('activatedFlag', this.activatedFlag);
  },
  ...
  methods: {
    ...
    handleLive() {
      let urlList = this.matchInfo.liveVideoUrlList;
      console.log('urlList', urlList);
      if (urlList != null && urlList.length > 0) {
        this.videoLiveInfo.videoUrl = urlList[0];
        this.videoLiveInfo.videoId = this.matchId;
        if (this.videoLiveInfo.videoUrl.includes('.m3u8')) {
          this.videoLiveInfo.videoType = 1;
        } else if (this.videoLiveInfo.videoUrl.includes('.flv')) {
          this.videoLiveInfo.videoType = 2;
        }
      }
    },
    /**
    ...
    openAndinitWebSocket() {
      ...
      return this.$post('/api/user/getUserWebsocket')
        .then(rsp => {
          ...
          this.socketGuess.socketObeject.initScene = initScene;
          console.log('更多玩法页', this.socketGuess.socketObeject);
        ...
        .then(rsp => {
          ...
          if (dataResponse.code == 200) {
            ...
            if (dataResponse.data.length > 0) {
              this.moreNoData = false;
            } else {
              this.moreNoData = true;
            }
          }
        })
        .catch(error => {
...
.allGuessPage {
  ...
  .guess_item {
    ...
    background-color: #3d3b51 !important;
  }
  .end_list {
    .title {
