<template>
  <div class="Page matchPage">
    <header class="mod_header">
      ...
      <navList></navList>-->
      <match-tab :currentTab='0'></match-tab>
      <div class="match_tab">
        <ul>
      ...
      <template v-else v-show="!isLoading">
        ...
        <mescroll ref="mescroll" :isShowEmpty="false" :downLoadAuto="false" @downCallback="downCallback"
          ...
          <section class="list_content" v-for="(item ,index) in dataList" :key="index" id='scrollId'>
            ...
            <div class="list" :id="item.sortDate | toDay">
              <div class="title">
                <div>
                  <!-- <img
...
import matchTab from "./components/tab.vue";
    ...
    getPageData(param) {
      ...
      return this.$post("/api/league/leagueIndexdata", param)
        .then(rsp => {
          ...
          if (dataResponse.code == "200") {
            this.quizShow = dataResponse.data.quizShow;
            if (param.loadType == 0) {
...
.list_content {
  ...
  .name {
    ...
    color: #000;
    // text-align: center;
    @include t_nowrap(25.3333vw);
    @media (max-width: 320px) {
      @include t_nowrap(71px);
    }
  }
  .score {
    @extend .flex_v_h;
