  ...
  <div>
    <div class="bet_item" v-if="guessEnd">
      <h3>
        ...
        <span>{{quizMathGame.subjectName}}</span><br>
      ...
      <div class="battle_team">
        ...
        <a v-for="(item,index) in quizMathGame.quizOptions" :key="index" :class="{select:getBetData.toCurBet == matchGameId+'_'+index,
          disabled:quizMathGame.suspended==1 ||quizMathGame.visible==0 || quizMathGame.status>=2}"
          @click="selectItem(item,index,$event)">
          <p class="answer">
            {{item.teamName|formatName(5)}}<i v-if="item.teamName.length>5">..</i> {{item.name}}</p>
    ...
    <div class="bet_item end_item" v-else>
      ...
      <div class="battle_team">
        <a v-for="(item,index) in quizMathGame.quizOptions" :key="index">
          <p class="answer">{{item.teamName|formatName(5)}}<i v-if="item.teamName.length>5">..</i> {{item.name}}
          </p>
          <span class="selected" v-if='item.quizzedCount>0'></span>
          <!-- v-if='item.quizzedCount>0||(item.index == userSelectItem.index && userSelectItem.quizzedCount>0)'></span> -->
          <!-- 赛果出来后的正确和错误 -->
          <span class="succeed" v-if="quizMathGame.awardResult==item.index"></span>
          <span class="failed" v-if="item.result=='R'|| quizMathGame.awardStatus==-1"></span>
...
.battle_team {
  ...
  a {
    ...
    font-size: 3.7333vw;
    border-radius: 6px;
    @include getRadiusBorder(#999, all, 12px);
