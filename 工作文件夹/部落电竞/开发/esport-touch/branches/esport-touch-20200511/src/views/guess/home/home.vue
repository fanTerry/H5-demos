    ...
    <mescroll ref="mescroll" @downCallback="downCallback" @upCallback="upCallback" @mescrollInit="mescrollInit"
      ...
      <div class="main" id="mainId">
        ...
        <section class="mod_live">
          ...
          <!-- 运营位 -->
          <banner ref="banner" class="banner" v-if="bannerUrlList.length > 0 " :adList='bannerUrlList' @toPop="toPop"></banner>
        </section>
        <!-- 跑马灯 -->
  ...
  methods: {
        ...
        this.leagueInfo.matchList.forEach((matchInfo, k) => {
          if (matchInfo.matchId == matchResponse.matchId) {
            ...
            if (data.matchType == "match_game") {
              ...
              // 保存要删除元素的索引 matchResponse.suspended == 1  matchResponse.visible == 0
              if (
                matchInfo.matchStatus != matchResponse.status &&
    ...
    runSpSchedule() {
      ...
      spSchdule = setInterval(() => {
        ...
        // }else{
        //   if (_this.leagueInfo.currSpList) {
        //   _this.updateMatchSp();
        // }
        // }
      }, _this.spUpdate.spUpdatTime * 1000);
    },
    /**
     * 定时页面更新赔率
     */
