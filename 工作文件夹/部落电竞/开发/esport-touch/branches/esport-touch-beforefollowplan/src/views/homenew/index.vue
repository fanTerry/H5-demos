<template>
  <div class="Page orangeGamePage">
    ...
    <mescroll id="mescroll" :class="{'video_bg':tabFlag == 1}" ref="mescroll" :isShowEmpty="false"
      ...
      <div class="main" id='mainId'>
        <div class="scroll_wrapper">
            ...
            <template v-for="(item,index) in newsList">
              ...
              <article class="item film_item" v-else-if="item.type==2" :key="index">
                <div class="film_con">
                  ...
                  <xi-gua-video ref="xiGuaVideo" :videoUrl="item.imageSrc" :poster="item.titleImg"
                    :videoId="item.aricleId" scene="home">
                  </xi-gua-video>
                </div>
              </article>
            </template>
          ...
          <!-- 没有数据时展示 -->
          <no-data v-if="noData" :text="'暂无新视频'" :imgUrl='require("../../assets/images/guess/no_data_icon.png")'>
          </no-data>
        </div>
      </div>
    </mescroll>
    <footer class="mod_footer" v-if='showFooterFlag'>
      <!-- <section class="AQ">
        ...
        .then(rsp => {
          ...
          if (dataResponse.code == '200') {
            if (this.tabFlag == 1) {
              ...
              if ((dataResponse.data.centerAdList.length > 0 || dataResponse.data.adList.length > 0) && this.adList.length <= 0) {
                this.adList = dataResponse.data.centerAdList;
                this.adList = this.adList.concat(dataResponse.data.adList);
                //如果是枫叶的banner在橘子电竞不显示
                this.adList = this.adList.filter(v => 
                  !(v.title.indexOf("枫叶")>-1&&location.toString().indexOf(".esportzoo.com") > -1)
                );
              }
            }
...
.infor_list .item {
  ...
  .item_img {
    ...
    object-fit: cover;
    border-radius: 5px;
    background-color: rgba(215, 58, 66, 0.04);
