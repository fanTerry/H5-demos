  ...
  <div class="Page guessRecord">
    ...
    <div class="main">
      <scroll ref="scroll" :scrollbar="scrollbar" :pullDownRefresh="pullDownRefresh" :pullUpLoad="pullUpLoad"
        :startY="0" @pullingUp="onPullingUp" @pullingDown="onPullingDown">
        <ul class="record_list">
          <li class="record_item" v-for="(item,index) in recordList" :key="index" @click="show(index)">
            ...
            <div class="bet_info">
              ...
              <span class="bingo" v-if="item.winStatus==2 && item.quizOrderStatus!=6 ">
                <i v-if="item.planType==3">+{{(sub(item.prize,add(item.winIncomeUser,item.winIncomeSys)))}}星星</i>
                <i v-else>+{{item.prize}}星星</i>
                派奖中
              </span>
            ...
            <div class="details" v-if="showDetails === index">
              ...
              <p v-if="item.winAward">
                中奖金额：
                <span>{{item.winAward}}</span>
              </p>
              <p v-if=" item.winIncomeUser && item.feeRate && item.commissionRate">
                佣金&nbsp;&nbsp;&nbsp;&nbsp;率：
                <span>{{item.feeRate + item.commissionRate}}%</span>
              </p>
              <p v-if="item.winIncomeUser">
                支付佣金：
                <span>{{add(item.winIncomeUser,item.winIncomeSys)}}</span>
              </p>
              <p>
                方案编号：
...
.recommend_btn {
  ...
  a {
    display: block;
    @include getBtn(auto, 9.0667vw, 4vw, #fff, $color_main, 1.3333vw);
  }
}
...
.confirm_btn {
  ...
  a {
    flex: 1;
    -webkit-flex: 1;
    @include getBtn(44.6667vw, 9.0667vw, 4vw, #fff, #999, 0.8vw);
    &:last-child {
      background-color: $color_main;
    }
  }
