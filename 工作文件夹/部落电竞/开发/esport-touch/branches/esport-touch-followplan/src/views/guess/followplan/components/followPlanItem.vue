<template>
  <!-- 已结束的单加end_order -->
  <li class="follow_order" :class="{'end_order':recommendDetail.status!=1}"
    @click=" detailType!=1 && goUrlPage('/followPlan/recommendPlanDetail')">
 ...
 :(recommendDetail.resultStatus==6 ||recommendDetail.resultStatus==5 ?'flow_tag':''))"></span>   
    <div class="user_info" v-if="detailType!=1">
      ...
      <div class="tips">
        <span v-if="recommendDetail.tenScoreStr">{{recommendDetail.tenScoreStr}}</span>
        <span>近十单收益：{{recommendDetail.tenReturnRate}}%</span>
      </div>
    </div>
