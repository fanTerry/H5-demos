<template>
  <div class="Page">
    ...
    <div class="main">
      <div class="match_bg" :style="'background-image: url('+(createMatchParam.gameImage||require('assets/images/matchtool/wzry_header_bg.png'))+')'">
      </div>
      <div class="create_match">
        <div class="item select_game">
          ...
          <div class="game_list" v-show="showGameList">
            <div :class="{cur:gameSelect == index}" v-for="(item,index) in infoData.allGames" :key="index" @click="toSelectGame(item,index)">
        ...
        <div class="item join_team">
          ...
          <div class="select_item" v-show="showTeamTotal">
            <span :class="{select:createMatchParam.teams == item}" v-for="(item,index) in infoData.teamsList" :key="index" @click="toSelectTeams(item)"><i></i>{{item}}</span>
          </div>
        </div>
        <div class="item">
        ...
        <div class="item race_round">
          ...
          <div class="item" v-show="showMatchRound" v-for="(item,index) in createMatchParam.rounds" :key="index">
            ...
            <div @click="pickDate(dateType.ROUNDTIME,index)">{{createMatchParam.roundTimes[index]||'填写时间'}}<i class="edit_icon"></i></div>
          </div>
        </div>
        <div class="item award_tips">
          <label for="award_tips">奖励说明</label>
          <textarea name="" v-model="createMatchParam.rewardDesc" maxlength="1000" id="award_tips" placeholder="请填写奖励机制" @click="scrollToBottom()" @blur="scrollToTop"></textarea>
        </div>
        <a class="next_btn" @click="checkMatchInfo">下一步</a>
      </div>
    </div>
    <vue-hash-calendar ref="picker" model="dialog" :scroll-change-date="true" :visible.sync="isShowCalendar" :default-datetime="defaultDatetime" :is-show-week-view="false" format="YY-MM-DD hh:mm"
      week-start="sunday" picker-type="datetime" :show-today-button="true" @confirm="dateConfirm">
    </vue-hash-calendar>
    <!-- 点击下一步弹出 -->
  ...
  methods: {
      ...
      if (curDateType == dateType.DEADLINETIME) {
        ...
        if (this.roundTimeIndex > 0) {
          ...
          if (date > this.createMatchParam.roundTimes[prevIndex]) {
            ...
            let tipMsg = '设置第' + (this.roundTimeIndex + 1) + '轮时间必须大于第' + (prevIndex + 1) + '轮时间';
            this.$toast(tipMsg, 4);
          }
        } else {
          //第一轮要大于比赛时间
    ...
    verifyParam() {
      ...
      for (var key in curParam) {
        ...
        let condition = (key == 'roundTimes' && curParam['rounds'] > 0 && curParam[key].length < curParam['rounds']) || !curParam[key];
