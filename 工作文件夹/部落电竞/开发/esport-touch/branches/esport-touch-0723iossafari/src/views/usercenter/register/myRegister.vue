<template>
  <div class="Page">
    ...
    <div class="main">
      ...
      <p class="rules">赢加竞技软件许可及服务协议，赢加竞技版权保护说明，用户私隐政策</p>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    afterLoginSuccess(member) {
      ...
      if (window.localStorage.getItem('toRedirectUrl')) {
        let toRedirectUrl = decodeURIComponent(window.localStorage.getItem('toRedirectUrl'));
        this.redirectUrl = toRedirectUrl.substring(toRedirectUrl.indexOf('com/') + 4);
