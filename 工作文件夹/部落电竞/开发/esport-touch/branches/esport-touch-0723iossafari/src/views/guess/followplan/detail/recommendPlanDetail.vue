    ...
    <div class='main' id='mainId'>
      <mescroll ref="mescroll" @downCallback="downCallback" @upCallback="upCallback" @mescrollInit="mescrollInit">
        ...
        <section class="bills_detail" v-if='recommendDetail'>
          <div class="mod_title">
            <span>发单详情</span>
            <span class="flex_hc">佣金率{{add(rate,commissionRate)}}%<i class="rules_icon" @click="popUps.showCommissionPop = true"></i></span>
          </div>
        ...
        <section class="bills_detail">
          ...
          <div class="match_table" :class="{end_table:recommendDetail && recommendDetail.status == 3}">
            <ul>
              <li class="title">
                <span>用户名</span>
                <span>跟单金额</span>
              ...
              <li v-for="(item,index) in planFollowList" :key="index">
                ...
                <span v-else>-</span>
                <span v-if="recommendDetail!=null && recommendDetail.resultStatus==2">{{(add(item.winIncomeUser,item.winIncomeSys)).toFixed(2)}}</span>
                <span v-else>-</span>
              </li>
              <li v-if="noData">
                <span>暂无跟单信息</span>
...
.bill_project {
  ...
  .bill_mask {
    ...
    border-radius: $border_radius;
    @include getBgImg('../../../../assets/images/followplan/default_img.png');
    background-size: contain;
...
.match_table {
  ...
  li {
    ...
    span {
      flex: 1;
      -webkit-flex: 1;
      border-left: 0.2667vw solid #353447;
      border-bottom: 0.2667vw solid #353447;
      font-size: 3.4667vw;
      line-height: 8.8vw;
      text-align: center;
      color: #fff;
