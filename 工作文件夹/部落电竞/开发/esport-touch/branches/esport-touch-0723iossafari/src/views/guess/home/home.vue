<template>
  <div class="Page guessPage" v-if='readyFlag'>
    <div class="main" id="mainId">
      <mescroll ref="mescroll" @downCallback="downCallback" @upCallback="upCallback" @mescrollInit="mescrollInit" :isShowEmpty="false">
        ...
        <ul class="guess_list" id="guess_list" v-if="leagueInfo.matchList.length != 0">
          <guess-item class="guess_item" :type=1 :matchInfo="item" v-for="(item,index) in leagueInfo.matchList" :key="index" :matchType="0" @toCharge="toCharge">
          </guess-item>
        </ul>
    ...
    <!-- 规则弹窗 -->
    <!-- <div class="ui_pop " v-if='showTips'> -->
    <div class="ui_pop " v-if='false'>
      <div class="pop_bill_rules">
        <h3>温馨提示<span class="close" @click="showTips=false"></span></h3>
        <div class="con">
          <p class="title">尊敬的客户您好！</p>
          <div>
            <p>
              由于近期频发的网络袭击安全事件，造成了赢加平台部分赛事服务永久性关停；
              经过夜以继日的抢救与性能修复，目前仍旧无法恢复正常服务。在诉诸司法手段追查网络攻击者的同时，
              我们只能很遗憾的通知您，<span>赢加平台将于6月25日晚24点正式停运</span>，期间我们会努力服务好各位的赢加商城奖励兑换工作。
            </p>
            <br>
            <p>
              大家可前往锦鲤商城或椰子商城完成奖励兑换
            </p>
            <br>
            <p>再次感谢您的大力支持，深表遗憾和歉意～</p>
          </div>
        </div>
        <a class="agree_btn" @click="goUrlPage('/userCenter')">去兑换</a>
      </div>
    </div>
    <!-- 首页焦点赛事-后续优化 -->
    ...
    <!-- 充值弹窗 -->
    <recharge-pop v-if="popUps.payPops.showRechargePop" @closeRechargePop="closeRechargePop" @getUserWallet="getUserWallet">
    </recharge-pop>
    <!-- 充值完成支付的弹窗,目前微信H5用上 -->
