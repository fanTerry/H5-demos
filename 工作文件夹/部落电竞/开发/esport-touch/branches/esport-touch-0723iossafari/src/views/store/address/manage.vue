<template>
  <div class="Page">
    ...
    <div class="main mescroll-touch-y">
      <div class="location_address" v-for="(item,index) in shopAddressList" :key="index">
        <div @click="toSelectAddress(item)">
          <p><span class="name">{{item.receiverName}}</span><span class="phone_num">{{item.receiverPhone}}</span><span class="default" v-if="item.defaulted==1">默认</span></p>
          <p class="address">{{item.receiverProvince}} {{item.receiverCity}} {{item.receiverDistrict}}
            {{item.receiverAddress}}</p>
        </div>
