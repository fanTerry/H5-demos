<template>
  <div class="ui_pop">
    <div class="pop_pay">
      ...
      <a class="confirm_btn" :href="fuiouSchemaUrl" v-if="choosedPayWay.payIndex==12&&fuiouSchemaUrl" @click="clickFuiouCharge()">{{buttonText}}</a>
  ...
  methods: {
    ...
    getFuiouSchemaUrl() {
      if (this.choosedPayWay.payIndex == this.globalConst.payIndexMap.get('FUIOU_ALI_PAY')) {
        let param = {
          chargeAmount: this.toChargePrice,
          chargeWay: this.choosedPayWay.payIndex
        };
        this.$post('/api/oauth2/authorize/alipay', param)
          .then(rsp => {
            ...
            if (dataResponse.code == '200') {
              let url = 'alipays://platformapi/startapp?appId=20000067&url=' + encodeURIComponent(dataResponse.data);
