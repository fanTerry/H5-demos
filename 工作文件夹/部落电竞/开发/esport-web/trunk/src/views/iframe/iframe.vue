<template>
  <div class="iframe" style="background:rgb(29,18,27)">
    <div class="iframe_header">
      <div class="user_info">
        ...
        <div class="login_in" v-else>
          <div class="user_assets">
            ...
            <span>{{getUserData.starNum}}星星</span>
          </div>
      ...
      <div class="right">
        ...
        <template v-if="showType == 0">
          <a class="exchange_btn" @click="creditsExchange">积分兑换</a>
          <a class="more_btn" @click="showType = 2">更多竞猜</a>
          <a class="rules_btn" @click="showType = 3">竞猜规则</a>
          <a class="record_btn" @click="quizzesRecord()">竞猜记录</a>
        </template>
    ...
    <div class="iframe_con">
      ...
      <!-- 竞猜记录 -->
      <div class="record" v-show="showType == 4">
        <!-- 数据加载出来前 -->
        <!-- <div class="loading" v-if="loadingType==0">
          <div class="loading_icon"></div>
          <p>正在加载中...</p>
        </div> -->
        <guess-record ref="myGuessRecord" :type="type" :recordType="recordType" :showGuessType="showGuessType"
          @closePop="showGuessType = false">
        </guess-record>
      </div>
    ...
    <exchange-pops :showType="exchangeType" @closePop="exchangeType = false">
    </exchange-pops>
  </div>
</template>

<script>
import loginPops from "../users/components/login_pops.vue";
import signPops from "../users/components/sign_pops.vue";
import rechargePops from "../users/components/recharge_pops.vue";
import guessLi from "./components/guess.vue";
import pops from "./components/pops.vue";

import guessRecord from "../users/components/guess_pops.vue";
import exchangePops from "../exchange/components/exchange_pops.vue";
  ...
  methods: {
    ...
    getLoginInfo() {
      ...
      if (this.userInfo != null) {
        return this.$post("/api/wxlogin/checkLogin")
          .then(rsp => {
    ...
    quizzesRecord(param) {
      ...
      if (this.checkLogin()) {
        ...
        this.type = 1;
        this.$refs.myGuessRecord.quizzesPopRecord(param);
      }
    },
...
.recharge_btn {
  ...
  background-clip: padding-box, border-box;
  background-origin: padding-box, border-box;
  background-image: linear-gradient(to bottom, #401b20, #401b20),
    linear-gradient(to bottom, #ba9d6e, #89642d);
  background-image: -webkit-linear-gradient(top, #401b20, #401b20),
    -webkit-linear-gradient(top, #ba9d6e, #89642d);
  background-image: -ms-linear-gradient(top, #401b20, #401b20),
    -ms-linear-gradient(top, #ba9d6e, #89642d);
...
.signin_btn {
  ...
  background-clip: padding-box, border-box;
  background-origin: padding-box, border-box;
  background-image: linear-gradient(to bottom, #37181f, #37181f),
    linear-gradient(to bottom, #115a7b, #115a7b);
  background-image: -webkit-linear-gradient(top, #37181f, #37181f),
    -webkit-linear-gradient(top, #115a7b, #115a7b);
  background-image: -ms-linear-gradient(top, #37181f, #37181f),
    -ms-linear-gradient(top, #115a7b, #115a7b);
  border: 2px transparent solid;
}
