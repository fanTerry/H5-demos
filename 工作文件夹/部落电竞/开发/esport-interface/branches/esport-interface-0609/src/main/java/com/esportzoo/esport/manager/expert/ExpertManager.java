		...
		try {
			...
			if (articlePageResult.isSuccess()) {
				...
				if (columnArticles != null) {
					logger.info("查询文章列表，pageNo={},pageSize={},articleQueryVo={},返回的结果集大小={}", articleDataPage.getPageNo(),
							articleDataPage.getPageSize(), JSON.toJSONString(articleQueryVo), columnArticles.size());
				}
...
//						ModelResult<List<Match>> listModelResult = matchServiceClient.queryMatchListByMatchIds(idList);//雷达数据源
						ModelResult<List<QuizMatch>> listModelResult = quizMatchServiceClient.queryListByIds(idList);//预测数据源
						List<QuizMatch> quizMatchList = listModelResult.getModel();
						if (listModelResult.isSuccess() && quizMatchList != null) {
                            List<QuizMatch> quizMatchResult = queryTeam(quizMatchList);
                            articleResponse.setQuizMatchResultList(quizMatchResult);
						}
					}
				}
	...
	private List<QuizMatch> queryTeam(List<QuizMatch> quizMatchList) {
        if (CollectionUtil.isEmpty(quizMatchList)) {
            return quizMatchList;
        }
        for (QuizMatch quizMatch:quizMatchList) {
            quizMatch.setAwayTeamLogo(application.getQuizAwayTeamLogo(quizMatch));
            quizMatch.setHomeTeamLogo(application.getQuizHomeTeamLogo(quizMatch));
            quizMatch.setHomeTeamName(application.getQuizHomeTeamName(quizMatch));
            quizMatch.setAwayTeamName(application.getQuizAwayTeamName(quizMatch));
        }
        return quizMatchList;
	}
}
