...
//			cmsContentDetailVo.setViews(cmsContentDetailVo.getComments() * 2 +cmsContentDetailVo.getViews() + idNum);
	...
	public CommonResponse<Boolean> upsOrCancel(@ApiParam(required = true, value = "1=点赞 0=取消点赞") int type,
		...
		try {
			...
			logger.info("用户：【{}】，点赞或取消点赞接口 【{}】毫秒",userConsumer.getId(), end-start);
...
//			return CommonResponse.withSuccessResp(cachedManager.getCacheSetSize(CacheType.CMS_CONTENT.getIndex(), cmsContent.getId() + ""));
	...
	@RequestMapping(value = "/getChatroomSwitch", produces = "application/json; charset=utf-8", method = RequestMethod.POST)
	@ApiOperation(value = "查询聊天室开关", httpMethod = "POST", consumes = "application/x-www-form-urlencoded", produces = "application/json")
	@ApiResponse(code = 200, message = "查询聊天室开关", response = CommonResponse.class)
	@ResponseBody
	public CommonResponse<Boolean> getChatroomSwitch(BaseRequest baseRequest) {
		boolean ChatroomSwitchType=false;
		try {
			/** 聊天室允许展示开关 */
			SysConfigProperty sysConfigPropertyByKey = getSysConfigByKey(SysConfigPropertyKey.CHATROOM_SWITCH, baseRequest.getClientType(),baseRequest.getAgentId());
			if (sysConfigPropertyByKey != null && StringUtils.isNotBlank(sysConfigPropertyByKey.getValue())) {
				String  ChatroomSwitch= sysConfigPropertyByKey.getValue();
				logger.info("聊天室拿到的开关:{}",ChatroomSwitch);
				if ("1".equals(ChatroomSwitch)) {
					ChatroomSwitchType=true;
				}
			}
		} catch (Exception e) {
			logger.info("查询查询聊天室开关：异常信息：【{}】", e.getMessage());
			e.printStackTrace();
			return CommonResponse.withErrorResp("查询聊天室开关异常");
		}
		return  CommonResponse.withSuccessResp(ChatroomSwitchType);
	}
}
