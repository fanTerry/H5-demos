...
import com.esportzoo.esport.constants.ThirdLoginStatus;
...
import com.esportzoo.esport.service.exception.errorcode.ThirdExchangeErrorTable;
...
public class YeYunOrderController extends BaseController {
	...
	@ResponseBody
	public CommonResponse<String> exchangeYeYunScore(@ApiParam(required = true, name = "兑换金额") BigDecimal walletScore, BaseRequest baseRequest, HttpServletRequest request){//渠道改为用户注册时渠道
		...
		try {
			...
			if (walletScore.compareTo(new BigDecimal(exchangeMix)) < 0){
				logger.info("椰子积分兑换，交易金额小于最小值 walletScore：{}",walletScore);
				throw new ExchangeException(ThirdExchangeErrorTable.EXCHANGE_SMALL_REC.code, ThirdExchangeErrorTable.EXCHANGE_SMALL_REC.msg+exchangeMix);
			}

			ModelResult<UserThirdLogin> thirdLoginModelResult = yeYunUserServiceClient.checkAndBind(userId);
			UserThirdLogin thirdLogin = thirdLoginModelResult.getModel();

			if (thirdLogin == null || thirdLogin.getStatus()!= ThirdLoginStatus.ACTIVE.getIndex()){
				logger.error("{}兑换椰云积分，第三方用户为空或状态异常：{}",logPrefix,JSON.toJSONString(thirdLogin));
				return CommonResponse.withErrorResp("用户状态异常");
			}
			//获取用户第三方ID
			String thirdUserId = thirdLogin.getThirdId();
			...
			userOperationParam.setChannelNo(userConsumer.getChannelNo());
			userOperationParam.setBizSystem(baseRequest.getBiz());
