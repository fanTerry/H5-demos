...
public class ThirdMallController extends BaseController {
    ...
    private static final String DUPLICATE_KEY_PREFIX = "interface_duplicate_key_userId_{}";
    ...
    public CommonResponse<KoiMallResponse> koiMall(BaseRequest baseRequest, HttpServletRequest request) {
        ...
        try {
            ...
            String key = StrUtil.format(DUPLICATE_KEY_PREFIX, userConsumer.getId());
            if (!redisClient.setNX(key, "1", 3)) {
                logger.error("用户[{}]提交频繁", userConsumer.getId());
                return CommonResponse.withErrorResp("请求频繁");
            }
            //锦鲤商城开关
            ...
            if (StringUtils.isBlank(openId)) {
                ModelResult<UserConsumer> result = userConsumerServiceClient.queryConsumerById(userConsumer.getId(), null);
                if (result.isSuccess() && result.getModel() != null) {
                    openId = result.getModel().getOpenId();
                } else {
