...
public class QuizBetController extends BaseController {
     ...
     * @return
     */
    @RequestMapping(value = "/judgeSystemInsideUser", produces = "application/json; charset=utf-8", method = RequestMethod.POST)
    @ApiOperation(value = "判断是否内部用户", httpMethod = "POST", consumes = "application/x-www-form-urlencoded", produces = "application/json")
    @ApiResponse(code = 200, message = "判断是否内部用户", response = CommonResponse.class)
    @ResponseBody
    public CommonResponse<Boolean> judgeSystemInsideUser(HttpServletRequest request, BaseRequest baseRequest) {
        try {
            UserConsumer userConsumer = getLoginUsr(request);
            if (userConsumer == null) {
                return CommonResponse.withErrorResp("未获取到登录用户信息");
            ...
            if (!userIds.contains(userId.intValue())) {
                logger.info("预测白名单[{}] 用户[{}]不存在之中", JSONObject.toJSONString(userIds), userId);
                return CommonResponse.withSuccessResp(false);
            }
            return CommonResponse.withSuccessResp(true);
        } catch (Exception e) {
            logger.error(logPrefix + "判断是否内部用户接口异常, 参数【{}】 异常【{}】", JSONObject.toJSONString(baseRequest), e.getMessage(), e);
            return CommonResponse.withErrorResp("判断是否内部用户异常");
        }
    }
}
