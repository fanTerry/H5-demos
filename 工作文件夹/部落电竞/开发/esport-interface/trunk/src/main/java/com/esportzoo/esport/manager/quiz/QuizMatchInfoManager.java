	...
	public DataPage<QuizMatchResponse> indexQuizMatch(QuizMatchInfoRequest matchInfoRequest, HttpServletRequest request, DataPage dataPage, UserConsumer userConsumer){
		...
		try {
			...
			for (QuizMatchResult quizMatchResult : sourceList) {
				...
				//设置首页直播
				if(quizMatchResult.getStatus() == QuizMatchStatus.GOING.getIndex()
						&& quizMatchResult.getLiveVisible() == LiveVisibleStatus.VISIBLE.getIndex() && StringUtils.isNotBlank(quizMatchResult.getLiveUrl())){
					matchResponse.setCanLive(LiveVisibleStatus.VISIBLE.getIndex());
				}
				//部分属性
