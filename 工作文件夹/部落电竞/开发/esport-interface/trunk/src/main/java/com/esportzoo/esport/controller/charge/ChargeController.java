...
import com.esportzoo.esport.connect.response.charge.ChargeAmountResponse;
...
import com.esportzoo.esport.hd.vo.HdGiftQueryVo;
...
import com.esportzoo.esport.util.WhiteUtils;
...
public class ChargeController extends BaseController {
	...
	public static final String logPrefix = "获取充值数据：";
	...
	@Autowired
	UserConsumerWhiteServiceClient userConsumerWhiteServiceClient;
	public static final String SEND_STAR_GIFT = "esport_interface_send_star_gift_num_";
	@Autowired
	...
	public CommonResponse<ChargePageResponse> getChargePageData(HttpServletRequest request, BaseRequest baseRequest) {
		try {
			...
			SysConfigProperty sysConfigProperty = getSysConfigByKey(SysConfigPropertyKey.WXXCY_CHARGE_AMOUNT_LIST, baseRequest.getClientType(),
					baseRequest.getAgentId());
			chargePageResponse.setChargeAmountList(sysConfigProperty.getValue().split(";"));

			List<ChargeAmountResponse> sendStar = getSendStar(chargePageResponse.getChargeAmountList(), userConsumer.getId());
			if (sendStar != null && sendStar.size()>0) {
				List<String> arrayList = Lists.newArrayList();
				for (ChargeAmountResponse chargeAmountResponse : sendStar) {
					arrayList.add(chargeAmountResponse.getChargeMoney());
				}
				chargePageResponse.setChargeAmountList(arrayList.toArray(new String[]{}));
			}

			chargePageResponse.setChargeAmountResponseList(sendStar);
	...
	public CommonResponse<ChargePageResponse> getIapAmountList(HttpServletRequest request, BaseRequest baseRequest) {
		try {
			...
			ChargePageResponse chargePageResponse = new ChargePageResponse();
			chargePageResponse.setAbleRecScore(userWallet.getAbleRecScore().toString());
			SysConfigProperty sysConfigProperty = getSysConfigByKey(SysConfigPropertyKey.IAP_CHARGE_AMOUNT_LIST, baseRequest.getClientType(),
					baseRequest.getAgentId());
	...
	@ResponseBody
	public CommonResponse<ChargeResponse> chargeRecScore(ChargeRequst chargeRequst, HttpServletRequest request) {//渠道改为用户注册时渠道
		try {
	...
	@ResponseBody
	public CommonResponse<H5ChargeResponse> charge(ChargeRequstExpand chargeRequst, HttpServletRequest request) {//渠道改为用户注册时渠道
		...
		try {
			...
			if (chargeRequst.getClientType() == null || !ClientType.getAllList()
					.contains(ClientType.valueOf(chargeRequst.getClientType().intValue()))) {
			...
			SysConfigProperty sysConfigProperty = getSysConfigByKey(SysConfigPropertyKey.WXXCY_CHARGE_AMOUNT_LIST, chargeRequst.getClientType(),
					new Long(chargeRequst.getAgentId()));
			List<String> chargeAmountList = Arrays.asList(sysConfigProperty.getValue().split(";"));
			if (!chargeAmountList.contains(chargeAmountStr)) {
				return CommonResponse.withErrorResp("充值金额不在充值列表项");
			}
			if (chargeRequst.getChargeWay() == null || !EsportPayway.getAllList()
					.contains(EsportPayway.valueOf(chargeRequst.getChargeWay().intValue()))) {
			...
			userOperationParm.setOperIp(RequestUtil.getClientIp(request));
			ModelResult<ChargePayRequest> modelResult = userChargeOrderServiceClient.insertUsrChargeOrder(order, userOperationParm);
			logger.info(logPrefix + "用户id={},用户昵称={},调用充值返回结果modelResult={}", userConsumer.getId(), userConsumer.getNickName(),
					JSON.toJSONString(modelResult));
			if (!modelResult.isSuccess()) {
				logger.info(logPrefix + "调用接口返回错误,errorCode={},errorMsg={},order={}", modelResult.getErrorCode(), modelResult.getErrorMsg(),
	...
	public CommonResponse<List<H5ChargeWay>> h5ChoosePayWay(HttpServletRequest request, BaseRequest baseRequest) {
		...
		try {
			...
			logger.info(logPrefix + "登陆用户id={},昵称={}，bizsystem={},当前是微信体系下[{}]", userConsumer.getId(), userConsumer.getNickName(),
					userConsumer.getBizSystem(), isWeiXinEnv);
			List<H5ChargeWay> resultList = new ArrayList<>();
			if (userManager.getBind(userConsumer.getId(), ThirdType.UBOX) != null) {
				SysConfigProperty sysConfigProperty = getSysConfigByKey(SysConfigPropertyKey.PAYMENT_CHANNEL_SWITCH_UBOX, baseRequest.getClientType(),
						baseRequest.getAgentId());
	...
	@RequestMapping(value = "/thirdPayCallBack/{payWay}", produces = "application/json; charset=utf-8")
	@ApiOperation(value = "第三方支付回调处理", httpMethod = "POST", consumes = "application/x-www-form-urlencoded", produces = "application/json")
	@ApiResponse(code = 200, message = "第三方支付回调处理", response = CommonResponse.class)
	@ResponseBody
	public CommonResponse<String> thirdPayCallBack(@PathVariable("payWay") Integer payWay, HttpServletRequest request, BaseRequest baseRequest, HttpServletResponse response) {
		String logPrefix = "第三方支付回调处理_";
		try {
			//可取request中第三方传回来的参数做相应的处理
			StringBuilder params = new StringBuilder();
			for(Object key:request.getParameterMap().keySet()){
				//logger.info("{}={}",key,request.getParameter((String)key));
				params.append(key.toString()).append("=").append(request.getParameter((String)key)).append("&");
			}
			logger.info(logPrefix + "支付方式[{}]，第三方传回来的参数[{}]", payWay, params.toString());
			//取出前端需要回跳的地址
			String redirectUrl = getRedirectUrl(payWay,request);
			logger.info(logPrefix + "需要回跳的地址{}", redirectUrl);
	 ...
	 * @return
	 */
	private String getRedirectUrl(Integer payWay,HttpServletRequest request){
		String redirectUrl = null;
		//默认跳转钱包流水页面
		String defaultRedirectUrl = touchAppDomain + "/wallet";
		if (payWay == null) {
			return defaultRedirectUrl;
		}
		String payNo = null;
		//TODO 每个第三方商户流水参数名不一致,先处理银联的 2020-05-29 11:35 linzheng
		if (EsportPayway.UMS_H5_PAY.getIndex() == payWay.intValue()) {
			payNo = request.getParameter("merOrderId");
		}
		String key = StrUtil.format(CacheKeyConstant.CHARGE_REDIRECT_URL_KEY, payNo);
		redirectUrl = redisClient.get(key);
	...
	private String getSign(String timeStamp, String nonceStr, String prepayId) {
		...
		logger.info("唤起微信支付参数timeStamp={},nonceStr={},prepayId={},appId={},signKey={},sign={}", timeStamp, nonceStr, prepayId, appId, signKey, sign);
		return sign;
	}
	...
	private List<ChargeAmountResponse> getSendStar(String[] chargeAmountList, Long userId) {

		ArrayList<ChargeAmountResponse> list = Lists.newArrayList();

		try {

			List<HdGift> hdGiftList = null;
			if (StringUtils.isNotEmpty(redisClient.get(SEND_STAR_GIFT))) {
				hdGiftList = redisClient.getObj(SEND_STAR_GIFT);

			} else {
				ModelResult<HdInfo> modelResult = hdInfoServiceClient.queryByCode(HdCode.HD_103.getIndex());
				HdInfo hdInfo = modelResult.getModel();
				HdGiftQueryVo queryVo = new HdGiftQueryVo();
				queryVo.setHdId(hdInfo.getId());
				queryVo.setStatus(HdGiftStatus.valid.getIndex());
				ModelResult<List<HdGift>> result = hdGiftServiceClient.queryByOption(queryVo);
				//ModelResult<List<HdGift>> result = hdGiftServiceClient.queryByHdId(hdInfo.getId());
				hdGiftList = result.getModel();
				redisClient.setObj(SEND_STAR_GIFT, 10 * 60, hdGiftList);

			}
			//过滤只取有效的
			List<HdGift> collect = hdGiftList.stream().filter(hdGift -> hdGift.getStatus() == HdGiftStatus.valid.getIndex())
					.collect(Collectors.toList());

			Map<String, BigDecimal> map = collect.stream().collect(Collectors.toMap(g -> g.getGiftPropJo().getString("money"), x -> x.getAmount()));
			logger.info("获得充值赠送金额列表" + "： 【{}】", JSON.toJSONString(map));

			List<String> objects = Lists.newArrayList();
			for (String chargeMoney : chargeAmountList) {
				ChargeAmountResponse amountResponse = new ChargeAmountResponse();
				amountResponse.setChargeMoney(chargeMoney);
				BigDecimal sendStar = map.get(chargeMoney);
				if (sendStar != null) {
					amountResponse.setSendStar(sendStar.intValue());
				} else {
					amountResponse.setSendStar(0);
				}
				//当用户不是系统用户，剔除小于1的充值项
				if (NumberUtil.isGreater(new BigDecimal("1"), new BigDecimal(chargeMoney))) {
					//判断用户是否为白名单中的系统内部用户
					List<Integer> userIds = whiteUtils.getUserIds(WhiteUtils.SYSTEM_INSIDE_USER);

					logger.info(logPrefix + "userid：{}，取出的内部用户：【{}】", userId, JSON.toJSONString(userIds));
					if (CollectionUtil.isEmpty(userIds) || !userIds.contains(userId.intValue())) {
						continue;
					}

				}
				objects.add(chargeMoney);
				list.add(amountResponse);
			}
			chargeAmountList = objects.toArray(new String[]{});


		} catch (Exception e) {
			logger.error("获取充值赠送星星出现异常", e);
			redisClient.del(SEND_STAR_GIFT);
			e.printStackTrace();
		}
