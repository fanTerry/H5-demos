...
public class QuizPlanRecommendController extends BaseController {
	...
	@RequestMapping("getRecommendRule")
	@ApiOperation(value = "发单规则", httpMethod = "POST", consumes = "application/x-www-form-urlencoded", produces = "application/json")
	@ApiResponse(code = 200, message = "发单规则", response = CommonResponse.class)
	public @ResponseBody CommonResponse<Dict> getRecommendRule(HttpServletRequest request) {

		Dict dict = redisClient.getObj( CacheKeyConstant.RECOMMEND_RULE );
		if (dict != null) {
			return CommonResponse.withSuccessResp( dict );
		}
		try {
			dict = Dict.create();
			/** [大神跟单]发起推单的最小投注金额 */
			SysConfigProperty property = sysConfigPropertyServiceClient.getSysConfigPropertyByKey( SysConfigPropertyKey.RECOMMEND_ORDER_BET_MONEY_MIN );
			String recommendOrderBetMoneyMin = property.getValue();
			dict.set( "moneyMin", recommendOrderBetMoneyMin );

			/** [大神跟单]发起跟单推单最低赔率 */
			property = sysConfigPropertyServiceClient.getSysConfigPropertyByKey( SysConfigPropertyKey.RECOMMEND_ORDER_BET_ODDS_MIN );
			String recommendOrderBetOddsMin = property.getValue();
			recommendOrderBetOddsMin = NumberUtil.formatPercent( new Double( recommendOrderBetOddsMin ), 1 );

			dict.set( "oddsMin", recommendOrderBetOddsMin );

			/** [大神跟单]单个用户当天可发的推荐单总数*/
			property = sysConfigPropertyServiceClient.getSysConfigPropertyByKey( SysConfigPropertyKey.TODAY_USER_ENABLE_RECOMMEND_ORDER_COUNT );
			String todayUserEnableRecommendOrderCount = property.getValue();
			dict.set( "orderCount", todayUserEnableRecommendOrderCount );

			/** [大神跟单]平台服务费率*/
			property = sysConfigPropertyServiceClient.getSysConfigPropertyByKey( SysConfigPropertyKey.RECOMMEND_SYSTEM_FEE_RATE );
			String recommendSystemFeeRate = property.getValue();
			dict.set( "feeRate", recommendSystemFeeRate+"%" );

			/** [大神跟单]发单人提成比例*/
			property = sysConfigPropertyServiceClient.getSysConfigPropertyByKey( SysConfigPropertyKey.RECOMMEND_USER_COMMISSION_RATE );
			String recommendUserCommissionRate = property.getValue();
			dict.set( "commissionRate", recommendUserCommissionRate+"%" );

			//缓存五分钟
			redisClient.setObj( CacheKeyConstant.RECOMMEND_RULE, CachedKeyAndTimeLong.setMinutes( 1 ), dict );
			return CommonResponse.withSuccessResp( dict );
		} catch (Exception e) {
			logger.error( logPrefix + "查询发单规则异常:{}", e );
			e.printStackTrace();
			return CommonResponse.withErrorResp( "查询发单规则异常" );
		}

	}

	public static void main(String[] args) {
		String	formatPercent = NumberUtil.formatPercent( new Double( 1.2 ), 1 );
