...
public class UserCenterManager {
	...
	@Autowired
	private ClientChannelInviteCodeServiceClient clientChannelInviteCodeServiceClient;
...
//			}
	...
	public ClientChannelInviteCode getUserInviteInfo(Long userId) {
		...
		try {
			...
			ModelResult<List<ClientChannelInviteCode>> listModelResult = clientChannelInviteCodeServiceClient.queryChannelInviteCode(queryVo);
			if (listModelResult != null && listModelResult.isSuccess() && listModelResult.getModel() != null) {
				if (listModelResult.getModel().size() > 0) {
					ClientChannelInviteCode clientChannelInviteCode = listModelResult.getModel().get(0);
					return clientChannelInviteCode;
				}
			}
		} catch (Exception e) {
			logger.error("用户：【{}】，获取销售管理权限出错 【{}】",userId,e);
			e.printStackTrace();
		}
