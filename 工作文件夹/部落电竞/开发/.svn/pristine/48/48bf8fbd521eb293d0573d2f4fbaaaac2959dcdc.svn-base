			...
			if (CollectionUtil.isNotEmpty(dataList)) {
				...
				for (QuizRecordResult quizRecordResult : dataList) {
					...
					if (quizRecordResult.getPlanType() == QuizPlanType.FOLLOW.getIndex()) {//是跟单方案
						ModelResult<QuizFollowRecordResult> followRecordResultModelResult = quizPlanFollowServiceClient.queryRecordByFollowPlanId(quizRecordResult.getId());
						if (!followRecordResultModelResult.isSuccess() && followRecordResultModelResult.getModel()==null) {
							logger.info(loggerPre + "跟单信息接口,调用接口返回错误,errorMsg={},FollowPlanId：【{}】", pageResult.getErrorMsg(), quizRecordResult.getId());
							continue;
						}
						QuizFollowRecordResult model = followRecordResultModelResult.getModel();
						//判断订单状态，未开奖不展示比赛相关信息
						if (quizRecordResult.getWinStatus()==QuizPlanWinStatus.NOT_OPEN.getIndex()){//未开奖
							quizRecordResponse.setRecommendAccount(model.getRecommendAccount());
							quizRecordResponse.setFollowUserNickName(model.getFollowUserNickName());
							...
							BeanUtil.copyProperties(model,quizRecordResponse, CopyOptions.create().ignoreNullValue());
						...
						getRecordInfo(playNoThirdNameMap, quizRecordResult, quizRecordResponse, matchGameInfo);
						if (quizRecordResponse.getWinStatus()==QuizPlanWinStatus.NOT_OPEN.getIndex() && quizRecordResponse.getStatus()== QuizPlanStatus.SUCCESS.getIndex()
						&& quizRecordResponse.getBetSp().compareTo(getSysTemRealBetOddsMin()) > -1 && quizRecordResponse.getCost().compareTo(getSysTemRealRecommedBetAmountMin()) >-1 ){
