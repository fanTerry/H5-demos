...
Component({
  ...
  properties: {
    initDuration: {
      type: null, // 可传入:Number(时间差单位秒) || String(当前系统时间)
  ...
  methods: {
    ...
    parseDate: function (date) {
      if (typeof date === 'string') {
        var format = date.replace(/-/g, '/');//ios系统不支持2018-03-29,替换成/
        return Math.floor((+new Date(format) / 1000)) - Math.floor((+new Date() / 1000))
      }
      return date
    }
  }
