...
export default {
  ...
  methods: {
    ...
    selectItem(item, index, e, locationType) {
      ...
      if (
        e.currentTarget.className == "match_item active" &&
        (e.target.className == "result_item" ||
          e.target.parentElement.className == "result_item")
      ) {
        this.setBetData({ toCurBet: null, locationType: locationType });
      }
    },
    //检查是否登录
    checkLogin() {
