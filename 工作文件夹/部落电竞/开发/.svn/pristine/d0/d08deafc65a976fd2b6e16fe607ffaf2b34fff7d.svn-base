<template>
  <div class='Page salesManagePage'>
    ...
    <div class='main' id="mainId">
      <mescroll ref="mescroll" @downCallback="downCallback" @upCallback="upCallback" @mescrollInit="mescrollInit" :isShowEmpty="false">
        ...
        <!-- <banner></banner> -->
        <div class="tab">
          <div @click="changeTab(5)" :class="{active:requestParam.queryType==5}">今日数据</div>
          <div @click="changeTab(6)" :class="{active:requestParam.queryType==6}">历史数据</div>
          <!-- <div>我的结算</div> -->
        </div>
        <div class="title_list">
          ...
          <div class="item">
            <p class="txt">充值</p>
            <p class="num recharge_num">{{sumData.scoreSum?sumData.scoreSum:0}} +
...
export default {
  ...
  methods: {
    ...
    changeTab(queryType) {
      ...
      this.requestParam.queryType = queryType;
      this.getIndexMatchData().then(data => {
        this.mescroll.endSuccess(this.currPageSize, this.loadMore);
      });
      this.getPersonSumData();
