...
public class QuizFollowPlanBetController extends BaseController {
    ...
    @ResponseBody
    public CommonResponse<Boolean> judgeCanRecommend(HttpServletRequest request, BaseRequest baseRequest, Long orderId) {
        try {
            UserConsumer userConsumer = getLoginUsr(request);
            if (userConsumer == null) {
                return CommonResponse.withErrorResp("未获取到登录用户信息");
            }
            return quizPlanRecommendManager.judgeCanRecommend(baseRequest, orderId);
        } catch (Exception e) {
