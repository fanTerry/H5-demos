<template>
  <div class="Page follow_Page">
    ...
    <mescroll ref="mescroll" @downCallback="downCallback" @upCallback="upCallback" @mescrollInit="mescrollInit">
      <div class="main" id='mainId' :class="{'blur':filterFlag}">
        <section class="mod_follow">
          ...
          <!-- banner图 -->
          <section class="banner" v-if="bannerUrlList.length > 0 ">
            <banner ref="banner" :adList="bannerUrlList" v-if="bannerUrlList.length > 0 && isKeep">
              <div class="banner_info" slot-scope="item">
                <p class="title">{{item.data.title}}</p>
                <!-- <p class="bottom">
                  <span class="logo">电竞</span>
                  <span class="watch_icon"><i></i>1592.5万</span>
                  <span class="edit_icon"><i></i>6.2万</span>
                  <span class="like_icon"><i></i>96.2万</span>
                </p> -->
              </div>
            </banner>
          </section>
          <!-- 文章列表组件 -->
          <article-tab :showType='showType' :cmsContentList='cmsContentList' :topCmsContentList='topCmsContentList'>
          </article-tab>
        </section>
    ...
    <publish-icon @filterBlur=filterBlur :topicId="'community'"></publish-icon>
        ...
        .then(rsp => {
          ...
          if (dataResponse.code == '200') {
            ...
            if (dataResponse.data.cmsContentList.length > 0) {
              this.cmsContentList = this.cmsContentList.concat(dataResponse.data.cmsContentList);
            }

            if (this.cmsContentList.length == 0 && this.topCmsContentList.length == 0) {
              this.noData = true;
            } else {
              this.noData = false;
            }
    ...
    getBannerUrl() {
      ...
      return this.$post('/api/getAdList', param)
        .then(rsp => {
          ...
          const dataResponse = rsp;
          if (dataResponse.code == 200) {
            console.log(dataResponse, '发现页广告');
            this.bannerUrlList = dataResponse.data.adList9;
          }
        })
        .catch(error => {
          console.log(error);
...
.main {
  position: relative;
  background: -webkit-linear-gradient(top, #fff 0%, #fff 44.2667vw, $color_main 42.1333vw, $color_main 100%);
  background: linear-gradient(to bottom, #fff 0%, #fff 44.2667vw, $color_main 42.1333vw, $color_main 100%);
  .nav_list {
    margin-bottom: 7px;
  }
  &.blur {
...
.banner_info {
  ...
  left: 0;
  width: 100%;
  padding: 2.1333vw 2vw 2.1333vw 3.2vw;
  ...
  .logo {
    ...
    font-size: 2.4vw;
    color: #fff;
    @include getBgImg('../../assets/images/home/game_icon.png');
  ...
  .like_icon {
    ...
    i {
      width: 3.2vw;
      height: 3.2vw;
      margin: 0 1.0667vw 0 2.4vw;
    }
  }
  .watch_icon i {
    @include getBgImg('../../assets/images/home/watch_icon.png');
    background-size: contain;
  }
  .edit_icon i {
    @include getBgImg('../../assets/images/home/edit_icon.png');
    background-size: contain;
  }
  .like_icon i {
    @include getBgImg('../../assets/images/home/like_icon.png');
    background-size: contain;
  }
}
