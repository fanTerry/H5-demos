        ...
        <div class="bet_num">
          <ul>
            ...
            <li class="custom" :class="{select:betObj.inputSelect}">
              <input placeholder="自定义" @click="handleInput($event)" @input="handleInput($event)" maxlength="7" type="tel" onkeypress="return (/[\d]/.test(String.fromCharCode(event.keyCode)))" @blur="inputBlur($event)" @focus="betObj.focusFlag=true" v-model.number.lazy="betObj.inputNum" @click.stop />
            </li>
          </ul>
        </div>
        <!-- 用户余额,单笔限额 -->
...
export default {
  ...
  methods: {
    ...
    betSuccess(data, e) {
      ...
      } else if (this.betObj.quizPlanStatus==2||this.selectItem.matchStatus == matchStatusJson.GOING) { //订单确认中或者赛中
...
.btn_box {
  margin-top: 3.2vw;
  @include getRadiusBorder(#ddd, top, 0);
}
