    ...
    <div class="main">
      <div class="container">
        ...
        <template v-if='loginType==2'>
          <phone-send-code ref='phoneSend' :cookieUseFlag='cookieFlag' :time="60" :btnTxt="'验证码登录'" @doConfirm="login"
        ...
        <p class="tar">
          ...
          <router-link to="/register" v-if='canRegister'>
...
export default {
  ...
  methods: {
    ...
    login(phone, phoneSendCode) {
      ...
      if (!this.cookieFlag) {
        this.$toast(
          "您的手机浏览器不支持或已经禁止使用cookie，无法正常登录，请开启或更换其他浏览器",
          5
        );
        return;
      }
