...
export default {
  ...
  methods: {
    judgeShowGuess() {
      ...
      if (clientType == 3 && (agentId == '200001' || agentId == '200002' || agentId == '200003' || agentId == '200004' || agentId == '200005'||agentId == '200006')) {
        this.showGuess = true;
      }
    },
    queryInviteCode() {
      this.$post('/api/user/queryInviteCode')
        .then(rsp => {
          ...
          if (dataResponse.code == '200' && dataResponse.data) {
            //0616判断域名包含
            if(location.toString().indexOf(".jinlibet.com") > -1||
              location.toString().indexOf(".fyesport.com") > -1 ||
              location.toString().indexOf(".ifesport.com") > -1) { 
              this.inviteCodeType = true;
            }else{
              console.log('域名不在显示范围',location.toString());
            }
