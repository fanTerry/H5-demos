<template>
  <div class="ui_pop" v-if="readyFlag">
    <div class="pop_pay">
      ...
      <div class="pay_method" v-for="(item,index) in payWay " @click="choosePay(item)" v-if='item.payIndex!=7&&item.payIndex!=8'>
        <div>
...
.pop_pay {
  ...
  .exchange_btn {
    ...
    padding: 0 30px;
    font-size: 17px;
    @include getBtn(auto, 40px, 17px, #fff, #d13840, 40px);
  }
}
</style>
