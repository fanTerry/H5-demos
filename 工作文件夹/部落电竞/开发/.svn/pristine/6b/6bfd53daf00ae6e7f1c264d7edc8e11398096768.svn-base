<template>
  ...
  <section class="mod_users">
    ...
    <!--顶部跑马灯-->
    <div class="broadcast" v-show="prizeList != ''">
...
export default {
  ...
  methods: {
    ...
    singout() {
      this.wxLoginout();
      // this.post("/api/wxlogin/wxLoginout")
      //   .then(dataResponse => {
    ...
    //竞猜记录弹窗
    quizzesRecord(param) {
      this.$refs.guess.toQuizRecord(param);
      this.type = 1;
    ...
    toExchangeRecode() {
      let param = {};
      return this.$post("/api/yeYunUser/getYeYunIndex", param)
        .then(rsp => {
          const dataResponse = rsp;
          if (dataResponse.code == 200) {
