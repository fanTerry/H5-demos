    ...
    <div class='main' id='mainId'>
      <mescroll ref="mescroll" @downCallback="downCallback" @upCallback="upCallback" @mescrollInit="mescrollInit">
        ...
        <section class="bills_detail" v-if='recommendDetail'>
          <div class="mod_title">
            <span>发单详情</span>
            <span class="flex_hc">佣金率{{rate}}%<i class="rules_icon" @click="popUps.showCommissionPop = true"></i></span>
          </div>
        ...
        <section class="bills_detail">
          ...
          <div class="bill_project">
            <!-- 需要填数据才能显示 -->
            <guess-item v-if="matchInfo!=null" :matchType="3" :isUserPlan='isUserPlan' :matchInfo="matchInfo">
            </guess-item>
            <!-- 遮罩 -->
          ...
          <div class="match_table">
            ...
            <ul>
              ...
              <li v-for="(item,index) in planFollowList" :key="index">
                ...
                <span v-if="recommendDetail.resultStatus==2">{{item.winAward.toFixed(2)}}</span>
                <span v-else>-</span>                
                <span v-if="recommendDetail.resultStatus==2">{{add(item.winIncomeUser,item.winIncomeSys)}}</span>              
                <span v-else>-</span>
              </li>
              <li v-if="noData">
                <span>暂无跟单信息</span>
      ...
      return this.$post('/api/planRecommend/recommendDetail', param)
        .then(rsp => {
          ...
          if (dataResponse.code == 200) {
            ...
            // this.rate=dataResponse.data.commissionRate+dataResponse.data.feeRate;
...
.follow_plan_btn {
  display: block;
  margin: 2.1333vw;
  @include getBtn(auto, 9.0667vw, 3.7333vw, #fff, $color_main, 0.8vw);
}
</style>
