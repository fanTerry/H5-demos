<template>
  <div class="Page userCenter_Page" @click="closeShowRulesPop($event)">
        ...
        <div class="swiper-container swiper_service">
          <ul class="swiper-wrapper">
            <li class="list swiper-slide">
              ...
              <div class="item" @click="goUrlPage2('/followPlan/myRecomendPlan')">
            ...
            <li class="list swiper-slide">
              ...
              <div class="item" @click="goUrlPage('/userCenter/salesExtend')">
                <img src="../../assets/images/user_center/extend_icon.png" alt="">
                <p>销售推广</p>
              </div>
      ...
      <section class="exchange_store">
        ...
        <div class="exchange">
          <div class="exchange_amount">
            ...
            <i class="rules_icon exchange_rules" v-if="userInfo.jinLiShopFlag" @click="popUps.showJlRulesPop = true"></i>
            <p class="yezi_title">
        ...
        <section class="store" v-for="(item,index) in adList " :key="index">
          <!-- <div class="store_item" @click='toJinLiMall()' v-if="userInfo.jinLiShopFlag">
            <img src="https://rs.esportzoo.com/upload/admin/cmsContent/2020/6/8/jinlinew_1591611616213.jpg" alt="">
            <span class="name">锦鲤商城</span>
          </div> -->
          <!-- <p class="yezi_title" v-if="item.title.indexOf('椰子') != -1 && adList.length>1">
            椰子分：<span>{{getVuexUserInfo.yeYunPoints}}</span>
            <i class="rules_icon yz_rules" @click="popUps.showYzRulesPop = true"></i>
            <a class="btn" @click="popUps.showExchangeType = true">椰子分兑换</a>
          </p> -->
          <div class="store_item" @click="goUrl(item.title)">
            <!-- <img src="../../assets/images/user_center/yzsc.png" alt="">
            <span class="name">椰子商城</span> -->
            <img :src="item.picUrl" alt="">
            <span class="name">{{item.title}}</span>
            <!-- 跑马灯 -->
            <section class="mod_award" v-if="prizeList.length>0 && adList.length==index+1">
    ...
    <recharge-pop v-if="popUps.showRechargePop" @closeRechargePop="popUps.showRechargePop = false" @getUserWallet="getUserWallet()"></recharge-pop>
...
export default {
  ...
  methods: {
    ...
    //跳转相应的商城
    goUrl(title) {
      if (title.indexOf('锦鲤') != -1) {
        this.toJinLiMall();
      } else if (title.indexOf('椰子') != -1) {
        this.goToYeYunPage();
      }
    },
    ...
    //预测banner广告
    getBannerUrl() {
      let param = { typeStr: '11' };
      return this.$post('/api/getAdList', param)
        .then(rsp => {
          this.readyFlag = true;
          const dataResponse = rsp;
          if (dataResponse.code == 200 && dataResponse.data.adList11) {
            this.adList = dataResponse.data.adList11;
          }
        })
        .catch(error => {
          console.log(error);
    ...
    getUserWallet() {
      return this.$post('/api/starNum/queryStarNum')
        .then(rsp => {
          ...
          if (dataResponse.code == 200) {
            let date = new Date();
            console.log(dataResponse.data.ableRecScore, '获取用户的星星,执行时间分秒' + date.getMinutes() + ':' + date.getSeconds());
      ...
      return this.$post('/api/quiz/record/broadcastList')
        .then(rsp => {
          ...
          if (dataResponse.code == 200) {
            ...
            this.prizeList = this.prizeList.filter(function(item) {
              return item.type == 2;
            });
          }
        })
