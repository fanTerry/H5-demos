...
export default {
  ...
  watch: {
    getBetData(val) {
      //当用户点击投注弹出，先暂停刷新
      console.log("val.toCurBet",val.toCurBet);
      if (val.toCurBet) {
          console.log("清楚");
        window.clearInterval(spSchdule);
      } else {
         if (window.location.href.indexOf("/guess/home")!=-1) {
           console.log("开始刷新");
           this.runSpSchedule();
         }
  ...
  methods: {
    ...mapActions(["setUserInfo","setBetData"]),
    /**
    ...
    refreshFirstData() {
      ...
      let param = {};
      param.pageNo = 1;
      param.pageSize = this.requestParam.pageNo * this.requestParam.pageSize;
      param.leagueId = this.requestParam.leagueId;
      param.videoGameId = this.requestParam.videoGameId;
      ...
      return this.$post("/api/quiz/match/indexQuizMatch", param)
        .then(rsp => {
          ...
          if (dataResponse.code == 200) {
            ...
            // let tempList = this.matchList.slice(this.requestParam.pageSize);
    ...
    dataRefresh() {
      ...
      this.getUserWallet();
      this.closeRechargePop();//关闭充值弹窗
      this.setBetData({ toCurBet: null });//清空选择的选项
    },
    // 跳转到椰云积分商城
    goToYeYunPage() {
