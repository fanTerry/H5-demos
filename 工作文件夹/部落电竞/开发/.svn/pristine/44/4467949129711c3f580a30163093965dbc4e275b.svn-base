 ...
 * @LastEditTime: 2020-07-15 15:06:39
  ...
  <div class="user_info">
    <div v-on:click.stop="toAuthorDetail(issueUserId)">
      ...
      <div>
        ...
        <p class="expert_tips">
          <span class="rate" v-if="follower.tenWinNum">{{follower.tenWinNum}}</span>
          <span class="award" v-if="follower.tenWinProfit">近十场收益：{{follower.tenWinProfit}}</span></p>
      </div>
    </div>
...
.user_img {
  ...
  .expert_player {
    ...
    @extend .g_c_mid;
    bottom: -0.8vw;
    @include getBtn(15.7333vw, 4.5333vw, 3.2vw, #fff, $color_btn, 34px);
    font-weight: bold;
  }
  .expert_player {
...
.expert_tips {
  ...
  margin-top: 2.1333vw;
  span {
    @include getBtn(auto, 4vw, 2.4vw, #fff, $color_btn, $border_radius);
