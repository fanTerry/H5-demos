...
public class ExpertArticleController extends BaseController {
	...
	public CommonResponse<RecOrderResponse> toPayArticle(@ApiParam(required=true, name="文章id")@PathVariable("articleId") Long articleId, 
			ArticlePayRequestParam payParam, HttpServletRequest request) {//渠道改为用户注册时渠道
		...
		try {
			...
			userOperationParam.setSellChannel(userConsumer.getChannelNo());
...
//				article.setViews(cachedManager.getCacheSetSize(CacheType.EPERT_ARTICEL.getIndex(),article.getId()+"").intValue());
	...
	@ApiOperation(value = "新的查询专家文章接口文章详情接口", httpMethod = "POST", consumes = "application/x-www-form-urlencoded", produces = "application/json")
	@ApiResponse(code = 200, message = "新的查询专家文章接口文章详情接口", response = CommonResponse.class)
	@ResponseBody
	public CommonResponse<ArticleDetailResponse> getDetailNew(@ApiParam(required = true, name = "文章id") @PathVariable("articleId") Long articleId, HttpServletRequest request) {
		try {
			...
			// 是否可以查看标识,默认为不能查看
			Boolean canView = Boolean.FALSE;
			if ((article.getIsFree().intValue() == ArticleFreeType.FREE.getIndex())
					|| (article.getPrice().compareTo(new BigDecimal(0)) <= 0)) {// 文章免费可以查看
				...
				if (article.getUserId().longValue() == userConsumer.getId().longValue()) {// 文章作者可以查看
					canView = Boolean.TRUE;
				}
				if (expertArticleManager.isPay(userConsumer.getId(), articleId)) {// 已付费可以查看
					canView = Boolean.TRUE;
				}
			}
			return CommonResponse.withSuccessResp(expertArticleManager.converToArticleDetail(article, canView));
		} catch (Exception e) {
	...
	public CommonResponse<PayOrderResponse> payArticle(@ApiParam(required = true, name = "文章id") @PathVariable("articleId") Long articleId,
			ArticlePaySubmitRequestParam payParam, HttpServletRequest request) {//渠道改为用户注册时渠道
		logger.info("根据文章id支付接口,接收到的参数articleId={},agentId={},choosedPayWay={}", articleId, payParam.getAgentId(), payParam.getChoosedPayWay());
