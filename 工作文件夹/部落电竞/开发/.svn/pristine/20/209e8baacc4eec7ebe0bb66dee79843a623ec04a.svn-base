  ...
  <div class="bet" :class="{'betAcross':betStyle==1,'betMore':betStyle==2}">
    <div>
      ...
      <div class="bet_num">
        ...
        <input id="bet_item" placeholder="自定义" @click="handleInput($event)" @input="handleInput($event)" maxlength="8"
          type="tel" autocomplete="off" onKeypress="return (/[\d]/.test(String.fromCharCode(event.keyCode)))" @blur="inputBlur($event)"
    ...
    <div>
      ...
      <a class="bet_now" @click="toShowConfirm"><span>立即竞猜</span></a>
    </div>
    <div class="ui_pop" v-if="showBetPop">
      ...
      <div class="confirm_order" v-if="confirmBetFlag">
        ...
        <h2>投注确认</h2>
        <h3>{{selectItem.homeTeamName}} <span>VS</span> {{selectItem.awayTeamName}}</h3>
        <p class='title'>[{{selectItem.matchNo | gameFightNum(true)}}] {{selectItem.subjectName}}</p>
        <div class="flex_v_justify">
          <p>方案内容：<span class='award'>{{selectItem.teamName}} {{selectItem.name}}</span></p>
          <p>投注金额：<span>{{userBetNum}}星星</span></p>
        </div>
        <div class="flex_v_justify">
          <p>奖励倍数：<span>{{selectItem.odds}}</span></p>
          <p>预计奖金：<span>{{(userBetNum * (selectItem&&selectItem.odds?selectItem.odds:0)).toFixed(0)}}星星</span>
          </p>
        </div>
        <a class="confirm_btn" @click="quizImmediately">确认</a>
      </div>
      <!-- 支付状态弹窗 -->
      <div class="betStatus" v-if='!confirmBetFlag'>
        <a class="close" @click="showBetPop=false"></a>
...
.bet {
  ...
  .bet_num {
    ...
    input {
      ...
      height: 46px;
      line-height: 46px;
      border-right: 1px solid #4e2c16;
      color: #fedcd7;
      text-align: center;
      &.active {
        color: #f5e3a5;
        background: linear-gradient(to right bottom, #f7d064, #8a5a26);
        background: -ms-linear-gradient(left top, #f7d064, #8a5a26);
        background: -webkit-linear-gradient(left top, #f7d064, #8a5a26);
      }
    }
    input {
      // flex: 1 auto;
