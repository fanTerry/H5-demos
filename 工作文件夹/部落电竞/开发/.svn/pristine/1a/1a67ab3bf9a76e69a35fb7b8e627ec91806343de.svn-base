          ...
          <div class="star_balance">
            <span>              
              <template v-if='getUserInfo.exchangeStarNum!=null'>
                {{getUserInfo.exchangeStarNum>getUserInfo.recScore?getUserInfo.recScore.toFixed(2):getUserInfo.exchangeStarNum.toFixed(2)}}
              </template>
            </span>
            <p>可兑换星星余额</p>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      //信息提示
      exchangeMsg: ['请输入正整数', '星星余额不足', '兑换星星最少为1', '兑换成功', '请输入1的倍数'],
      tabCheck: 0,
  ...
  methods: {
    ...
    inputNum(e) {
      ...
      } else if (this.starNum >= this.multi && this.starNum <= this.getUserInfo.exchangeStarNum && this.starNum % this.multi == 0) {
        console.log('数字合适');
        this.showNum = true;
    ...
    allExchange() {
      if (this.getUserInfo.exchangeStarNum > this.getUserInfo.recScore) {
        this.starNum = parseInt(this.getUserInfo.recScore);
      } else {
