      ...
      <template v-if="!noData">
        <mescroll ref="mescroll" @downCallback="downCallback" @upCallback="upCallback" @mescrollInit="mescrollInit">
          <section class="myfollow_tab">
            <!-- 粉丝列表 -->
            <div class="user_list">
              <user-info v-for="(item,index) in followList" :key="index" :follower="item" :issueUserId="item.userId"
                @follow='follow'></user-info>
            </div>
          </section>
        </mescroll>
      </template>
...
export default {
  ...
  methods: {
    ...
    getPageData(param) {
      ...
      return this.$post("/agency/user/pageFollowedUser", param)
        .then(rsp => {
          const dataResponse = rsp;
          if (dataResponse.code == "200") {
            console.log(dataResponse, "粉丝首页--getPageData--请求成功");
    ...
    updateFollowStatus(userId, status) {
      ...
      return this.$post("/agency/user/follow", param)
        .then(rsp => {
          const dataResponse = rsp;
          if (dataResponse.code == "200") {
            this.$toast(message);
