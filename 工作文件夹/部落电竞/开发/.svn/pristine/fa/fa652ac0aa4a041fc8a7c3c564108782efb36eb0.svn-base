  ...
  <div class="Page userAuthPage">
    ...
    <div class="main">
      ...
      <article class="bind_card_rules">
        ...
        <p>2.一旦完成个人银行卡号填写，枫叶个人账户余额将会冻结，无法进行任何货币操作；</p>
        <p>3.枫叶余额退款采用人工手动打款方式进行，因此需一定的人工处理时间，平台将会努力保证在您填写完24小时内完成打款，请注意银行收款短信信息；</p>
...
export default {
  ...
  methods: {
    getUserBankCard() {
      this.$post('/agency/usercenter/getBankCard')
        .then(rsp => {
    ...
    toSubmit() {
      ...
      this.$post('/agency/usercenter/bindCard', this.bankInfo)
        .then(rsp => {
          let response = rsp.data;
          if (rsp.code === '200') {
            this.$toast('银行卡绑定成功!');
