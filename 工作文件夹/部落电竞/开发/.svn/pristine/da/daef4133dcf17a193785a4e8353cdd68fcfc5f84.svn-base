      ...
      <section class="my_service">
        ...
        <div class="swiper-container swiper_service">
          <ul class="swiper-wrapper">
            ...
            <li class="list swiper-slide" v-if="userInfo.saleAuth">
        ...
        <div class="exchange">
          <p class="exchange_amount">可兑换商品额度：<span>
              <template v-if='getVuexUserInfo.exchangeStarNum!=null'>
                ...
                {{getVuexUserInfo.exchangeStarNum.toFixed(2)}}
              </template>
            </span>
...
export default {
  ...
  methods: {
    ...
    judgeSystemUser() {
      this.$post('/agency/quiz/bet/judgeSystemInsideUser')
    ...
    //查询设置用户钱包
    getUserWallet() {
      return this.$post('/agency/starNum/queryStarNum')
        .then(rsp => {
          const dataResponse = rsp;
          if (dataResponse.code == 200) {
    ...
    //查询椰云积分
    queryYeYunPoints() {
      return this.$post('/agency/yeYunUser/yeYunUserInfo')
        .then(rsp => {
          const dataResponse = rsp;
          if (dataResponse.code == 200) {
    ...
    //查询用户可兑换余额
    getExchangeStarNum() {
      this.$post('/agency/starNum/queryExchangeScore')
        .then(rsp => {
    ...
    // 跑马灯数据请求
    prizeCarousel() {
      return this.$post('/agency/quiz/record/broadcastList')
        .then(rsp => {
          const dataResponse = rsp;
          if (dataResponse.code == 200) {
    ...
    //   this.$post("/agency/user/checkBindingPhone")
    //     .then(dataResponse => {
