    ...
    <div class="main" id='mainId'>
      <mescroll ref="mescroll" @downCallback="downCallback" @upCallback="upCallback" @mescrollInit="mescrollInit">
        <section class="mod_follow">
          ...
          <!-- 文章列表组件 -->
          <articleTab :showType='showType' :videoType="2" :showCommon="1" :cmsContentList='cmsContentList' :topCmsContentList='topCmsContentList'>
          </articleTab>
        </section>
...
export default {
  ...
  methods: {
    ...
    getPageContenList(param) {
      ...
      console.log(param);
      return this.$post('/api/userMsg/cmsMsg', param)
        .then(rsp => {
          const dataResponse = rsp;
          console.log(dataResponse.data.cmsContentList);
          if (dataResponse.code == '200') {
            if (dataResponse.data.dataList != null && dataResponse.data.dataList.length > 0) {
