        ...
        <ul class="record_list">
          <li class="record_item" v-for="(item,index) in recordList" :key="index" @click="show(index)">
            ...
            <div class="bet_info">
              ...
              <!-- quizOrderStatus==6派奖或9重派奖（前端不做区分展示） -->
              <span class="bingo" v-if="item.winStatus==2 &&  item.quizOrderStatus!=6 && quizOrderStatus!=9 ">猜中+{{item.prize}}星星 派奖中</span>
              <span class="bingo" v-else-if="item.winStatus==2 && (item.quizOrderStatus==6 || quizOrderStatus==9)  ">猜中+{{item.prize}}星星 已派奖</span>
