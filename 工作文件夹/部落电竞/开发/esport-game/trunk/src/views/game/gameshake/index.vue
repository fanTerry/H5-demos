    ...
    <div class='main'>
      <div>
        <div class="tree">
          ...
          <!-- 奖品掉落球 -->
          <img class="prize_img" :class="{'active':gameShake.gameBingo}" @animationend="ballDropOut()" src="../../../assets/images/game/gameshake/ball.png" alt="" style="">
          <!-- 中奖物品 -->
          <template v-if='gameShake.playRes'>
            <div class="awards" :class="{'more_awards':gameShake.playRes.returnAmt/gameShake.playRes.betAmt>1}">
              <div class="award_packs" :class="{'active':gameShake.showAwardBg}">
                <!-- 中奖星星 -->
                <div class="award_content" :class="{'active':gameShake.ballDrop}" v-for="(award,index) in gameShake.playRes.awardList" :key="index">
                  <img :src="award.icon | gameFruits">
                  <span>X {{award.num}}</span>
                  <span class="fireworks"></span>
                </div>
              </div>
              <p class="award_num" :class="{'active':gameShake.ballDrop}">+{{gameShake.playRes.returnAmt}}</p>
            </div>
          </template>
          <!--背景音乐-->
          ...
          <div class="play">
            <div class="bet" v-if='roomInfo.playItems'>
              <div class="power" @click="showPowerList( $event)">
                ...
                <ul v-if="shutFlag">
                  <li>摇晃力量</li>
                  <li v-for="(item,index) in roomInfo.playItems" :key="index" :class="{'active':roomInfo.selectPlayItem == item}" @click="selectPower(index,$event)">
                    {{item}}
                  </li>
                </ul>
                <span class="num">{{roomInfo.selectPlayItem}}</span>
  ...
  data() {
    return {
      ...
      gameShake: {
        ...
        // songshu: require("../../../assets/images/game/gameshake/songshu.png")
      },
      roomInfo: {
        //房间信息
        playItems: null, //房间档位
        ...
        audioSrc: ['http://rs.esportzoo.com/upload/game/bg.mp3', 'http://rs.esportzoo.com/upload/game/bit.mp3', 'http://rs.esportzoo.com/upload/game/win.mp3']
      },
      betFailedFlag: false
    };
  },
  ...
  methods: {
      ...
      this.$post('/agency/api/gameRoom/roominfo', form)
        .then(res => {
          ...
          if (res.code == 200) {
            ...
            // this.roomInfo.selectPlayItem = res.data.defaultPlayItem;
            this.roomInfo.selectPlayItem = this.roomInfo.playItems[0];
          } else {
          }
        })
        .catch(error => {
    ...
    playGame(autoPlay) {
      ...
      if (this.roomInfo.isSubmit || this.gameShake.gameStatus) {
        // this.$toast("游戏进行中");
        console.log('this.roomInfo.isSubmit=' + this.roomInfo.isSubmit + ',this.gameShake.gameStatus=' + this.gameShake.gameStatus, '游戏进行中');
        return;
      } else {
        ...
        let playData = new FormData();
        playData.append('gamecode', 'YAOYY');
        playData.append('roomid', '');
        playData.append('betamt', this.roomInfo.selectPlayItem);
        // let playData = {
        //   gamecode: 'YAOYY',
        //   // roomid: this.roomInfo.roomNo,
        //   roomid: '',
        //   betamt: this.roomInfo.selectPlayItem
        //   // autoPlay: autoPlay || false
        // };
        this.doSubmitPlay(playData).then(res => {
          console.log(res, '调用提交游戏数据接口');
          this.roomInfo.isSubmit = false;
          if (res.code == '200') {
            // if (!res.data.balanceEnough) {
            //   this.betFailedFlag = true;
            //   return;
            // }
            //游戏成功处理
            this.playSuccess(res);
          } else {
            //游戏失败处理
    ...
    treeShakeEnd() {
      ...
      // this.gameShake.songshu = require("../../../assets/images/game/gameshake/songshu.png");
    ...
    doSubmitPlay(playData) {
      return this.$post('/agency/api/gameOrder/addOpenNow', playData)
        .then(data => {
          return data;
        })
        .catch(error => {
    ...
    playSuccess(res) {
      ...
      //   require("../../../assets/images/game/gameshake/songshu.gif") +
      //   "?" +
      //   Math.random();
      console.log(res);
      ...
      this.gameShake.playRes = res.data;
      let useStar = this.roomInfo.selectPlayItem;
    ...
    //检查账户余额
    checkBalance() {
      let userPowerNum = this.roomInfo.selectPlayItem;
      if (!userPowerNum) {
        this.$toast('请先选择摇晃力量~');
        return false;
      }
    ...
    iosAutoPlayBgm() {
      ...
      // this.$post('/agency/wxlogin/getJsConfig')
      //   .then(rsp => {
      //     console.log(rsp, 'rsp');
      //     if (rsp.appId != '') {
      //       this.wx.config({
...
.Page {
  ...
  &::before {
    ...
    width: 100%;
    height: 100%;
    @include getBgImg('../../../assets/images/game/gameshake/shake_bg.png');
...
.gold_coin {
  ...
  span {
    ...
    width: 12vw;
    height: 12vw;
    @include getBgImg('../../../assets/images/game/gameshake/star_icon.png');
  }
}
...
.tree {
  ...
  .tree_leaf {
    ...
    width: 100%;
    height: 90.9333vw;
    @include getBgImg('../../../assets/images/game/gameshake/tree_leaf.gif');
    // &::after {
    //   content: "";
    //   @extend .g_c_mid;
    //   top: 35.2vw;
    ...
    //   width: 44.5333vw;
    //   height: 10vw;
    //   @include getBgImg("../../../assets/images/game/gameshake/tree_name.png");
    // }
  }
  .tree_back {
    z-index: 1;
    ...
    width: 78.1333vw;
    height: 26vw;
    @include getBgImg('../../../assets/images/game/gameshake/leaf_back.png');
  }
  .tree_body {
    @extend .g_c_mid;
    bottom: 2.6667vw;
    ...
    @include getBgImg('../../../assets/images/game/gameshake/tree_body.png');
  }
  .prize_img {
    position: absolute;
    top: 20%;
  ...
  .award_packs {
    ...
    margin: 0 auto;
    &.active {
      @include getBgImg('../../../assets/images/game/gameshake/shine.gif');
  ...
  .more_awards .award_packs {
    ...
    height: 77.3333vw;
    &.active {
      @include getBgImg('../../../assets/images/game/gameshake/shine_strong.gif');
    }
  }
  .award_num {
    position: absolute;
  ...
  .award_content {
    ...
    &.active {
      .fireworks {
        ...
        width: 42.6667vw;
        height: 42.6667vw;
        @include getBgImg('../../../assets/images/game/gameshake/fireworks.gif');
        background-size: 100% auto;
      }
    }
    ...
    &:nth-of-type(2) {
      ...
      &.active {
        ...
        .fireworks {
          animation: changeBg 0.5s linear 0.5s forwards;
        }
      }
    }
    &:nth-of-type(3) {
    ...
    &:nth-of-type(4) {
      ...
      &.active {
        ...
        .fireworks {
          animation: changeBg 0.5s linear 1.5s forwards;
        }
      }
    }
    ...
    &:nth-of-type(5) {
      ...
      &.active {
        ...
        .fireworks {
          animation: changeBg 0.5s linear 2s forwards;
        }
      }
    }
    ...
    &:nth-of-type(6) {
      ...
      &.active {
        ...
        .fireworks {
          animation: changeBg 0.5s linear 2.5s forwards;
        }
      }
    }
    span {
  ...
  .songshu {
    ...
    width: 22.5333vw;
    height: 40.9333vw;
    @include getBgImg('../../../assets/images/game/gameshake/songshu_await.png');
    ...
    &.active {
      width: 25.7333vw;
      height: 39.4667vw;
      @include getBgImg('../../../assets/images/game/gameshake/songshu_sprite.png');
      background-position: 0 0;
...
.voice {
  ...
  width: 14.9333vw;
  height: 14.9333vw;
  @include getBgImg('../../../assets/images/game/gameshake/voice.png');
  &.muted {
    @include getBgImg('../../../assets/images/game/gameshake/voice_forbidden.png');
  }
}
...
.mod_footer {
  position: relative;
  z-index: 5;
  @include getBgImg('../../../assets/images/game/gameshake/footer_bg.png');
  .play {
    @extend .flex_hc;
    height: 24vw;
  ...
  .bet_star {
    ...
    width: 7.4667vw;
    height: 7.4667vw;
    @include getBgImg('../../../assets/images/game/gameshake/bet_star.png');
  }
  .open {
    position: absolute;
    z-index: 10;
    ...
    width: 9.6vw;
    height: 9.6vw;
    @include getBgImg('../../../assets/images/game/gameshake/open.png');
    &.shut {
      @include getBgImg('../../../assets/images/game/gameshake/shut.png');
    }
  }
  .power {
    position: relative;
    ul {
      ...
      li {
        ...
        text-align: center;
        color: #fff;
        @include getBgImg('../../../assets/images/game/gameshake/shangla_off.png');
        border-radius: 8.8vw;
        &:first-child {
          width: 100%;
          height: 10.6667vw;
        ...
        &.active {
          font-weight: bold;
          color: #f64500;
          @include getBgImg('../../../assets/images/game/gameshake/shangla_on.png');
        }
      }
    }
    .num {
      ...
      font-size: 4.5333vw;
      color: #fff;
      @include getBgImg('../../../assets/images/game/gameshake/shangla_bg.png');
    }
  }
  .play_btn {
    position: relative;
    ...
    @include getBgImg('../../../assets/images/game/gameshake/btn_bg.png');
    background-size: 100% auto;
    a {
      position: absolute;
      bottom: 0;
      ...
      width: 100%;
      height: 100%;
      @include getBgImg('../../../assets/images/game/gameshake/play_btn.png');
      background-size: 100% auto;
      background-position: center bottom;
      &.active {
  ...
  .treasure_box {
    ...
    height: 9.3333vw;
    margin: 0 2.4vw;
    @include getBgImg('../../../assets/images/game/gameshake/treasure_box.png');
    span {
      @extend .g_c_mid;
      bottom: 0.6667vw;
      padding: 0 1.6vw;
  ...
  .game_hook {
    ...
    text-align: center;
    color: #fff;
    @include getBgImg('../../../assets/images/game/gameshake/game_hook.png');
    &.active {
      @include getBgImg('../../../assets/images/game/gameshake/game_hooking.png');
    }
    span {
      @extend .g_c_mid;
      top: -6.6667vw;
      ...
      white-space: nowrap;
      color: #fff;
      @include getBgImg('../../../assets/images/game/gameshake/hook_tips.png');
    }
  }
}
