...
router.beforeEach((to, from, next) => {
    ...
    var language = to.query.locale ? to.query.locale : 'zh';  //获取语言标识，设置对应的显示语言
    ...
    if (biz && biz == 2) { //友宝业务系统
        ...
        if (!cookieSid || cookieSid == "" || cookieSid == '""') {
            ...
            window.location.href = "/agency/third/login?agentId=" + agentId + "&biz=" + biz + "&redirect=" + encodeURIComponent(window.location.href);
            return;
        } else {
    ...
    if (clientType == 7) { //微信公众号进入
        ...
        if (!cookieSid || cookieSid == "" || cookieSid == '""') {
            ...
            window.location.href = "/agency/wxlogin/toAuth?agentId=" + agentId + "&biz=" + biz + "&clientType=7" + "&backUrl=" + encodeURIComponent(window.location.href);
            return;
        }
    }
